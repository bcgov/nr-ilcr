...
public class SecurityConfiguration {
   ...
   * @param cognitoGroupsConverter the JWT converter
    ...
    if (securityEnabled) {
      http.oauth2ResourceServer(
          ...
          .authorizeHttpRequests(
              authorize ->
                  authorize
                      .requestMatchers(PATH_HEALTH, PATH_INFO)
                      .permitAll()
                      .requestMatchers(PATH_HEALTH, PATH_HEALTH + "/**", PATH_INFO)
                      .permitAll()
                      .requestMatchers(HttpMethod.GET, BackendConstants.HOME_PUBLIC_PATHS)
                      .permitAll()
                      .requestMatchers("/api/**")
                      .authenticated()
                      .anyRequest()
