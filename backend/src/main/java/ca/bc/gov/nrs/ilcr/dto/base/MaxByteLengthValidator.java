 ...
 *
 * <p>{@code null} passes: presence is {@code @NotBlank}'s job, and a nullable optional field must
 * not acquire a required-ness it never had. Not trimmed either — the value measured here is the
 * value that will be bound, and Schedule 5 trims {@code campName} in the service (deviation (I))
 * AFTER validation, so trimming here would let a padded 32-byte name through to a 30-byte column on
 * any path that skipped the trim.
 *
