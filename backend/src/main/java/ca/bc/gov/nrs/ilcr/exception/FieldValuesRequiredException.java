 ...
 * {@link GlobalExceptionHandler} can resolve the verbatim legacy required-field text ({@code
 * javax.faces.component.UIInput.REQUIRED = "{0}: Value is required."}) once per field and return
 * ALL messages together on a single 400 (S08) — unlike a typed {@code @RequestParam}, which would
 * fail on the first field only.
 */
