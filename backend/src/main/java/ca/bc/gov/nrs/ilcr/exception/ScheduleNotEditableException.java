...
/**
 * Raised when a write (PUT/DELETE) targets a schedule whose Schedules 1–10 track is not in Draft — the
 * server-side Draft gate (AD-9). Legacy enforced this only by disabling the UI, so no legacy message
 * text exists and the key text is a recorded deviation (AD-8). Maps to 409.
 *
