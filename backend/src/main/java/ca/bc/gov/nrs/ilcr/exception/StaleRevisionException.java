 ...
 * {@code ILCR_REPORT_SUMMARY.REVISION_COUNT} — a lost-update conflict (AR11 optimistic lock): another
 * user saved in between, so the optimistic-lock bump matched 0 rows. Maps to 409 with the verbatim
 * {@code scheduleRevisionConflictErrorMsg}. No legacy message text existed (legacy used Hibernate
