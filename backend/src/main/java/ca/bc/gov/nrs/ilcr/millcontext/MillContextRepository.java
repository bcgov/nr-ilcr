 ...
 * viewable (AD-3: Spring Data JDBC — repository interface + {@code @Table} record entity {@link
 * MillStatusXref} + explicit {@code @Query} named-parameter SQL). SQL only — decisions live in
 * {@link MillContextService}. Spring Data JDBC access to the legacy {@code THE} tables that
 * determine whether a mill/reporting-year context is viewable (AD-3: repository interface +
 * {@code @Table} record entities + explicit {@code @Query} named-parameter SQL; migrated from the
 * retired {@code JdbcClient} idiom, behavior preserved). SQL only — decisions live in {@link
 * MillContextService}; entities never cross the service boundary (the {@code default} methods below
 * map them to DTO / projection records).
 *
...
public interface MillContextRepository extends Repository<SelectableMillEntity, Long> {
   ...
   * must have its one-to-one {@code ILCR_MILL_STATUS_XREF} row AND at least one {@code
   * ILCR_MILL_REPORT_STATUS} row (any year, i.e. ever enrolled in reporting) to be listed
  ...
  /**
   * Backing read for {@link #findAllMills()}. The status code comes from {@code
   * THE.ILCR_MILL_STATUS_XREF}, whose PK {@code ILCR_MILL_STATUS_XREF_ID} is the one-to-one mill id
   * (legacy {@code ILCRMillStatusXref} maps {@code @OneToOne @PrimaryKeyJoinColumn} to {@code
   * Mill}). {@code MILL_ID} tiebreaker keeps equal/NULL mill numbers deterministically ordered.
   * Columns map to {@link SelectableMillEntity} by their {@code THE} names.
   */
  @Query(
      """
  ...
  /**
   * The opened reporting years for the Home page selection list — every existing {@code
   * THE.ILCR_REPORTING_PERIOD} row — ordered by {@code REPORT_YEAR} descending (BR-03, legacy
   * {@code getReportingPeriods()}).
   *
   ...
   * selectable (Story 1.2 resolves that to 404, matching what legacy's server-controlled dropdown
   * made unreachable).
   *
  ...
  /**
   * The per-status display-date strings for a (mill, year) pair from {@code
   * THE.ILCR_MILL_REPORT_STATUS_RPT_VW} (a VIEW on the delivery DB; the test snapshot stands it in
   * as a table — see {@code V6}). Values carry the legacy 3-character prefix; stripping is the
   ...
   * {@code ILCR_MILL_REPORT_STATUS_RPT_VW} is a VIEW with no PK/uniqueness guarantee: legacy read
   * it as a list and took {@code get(0)} ({@code MillReportStatusDAO.getMillReportStatusList}), so
   * a multi-row view must resolve to the first row, not throw {@code
  ...
  /**
   * Projection: the raw (still-prefixed) per-status date strings of one {@code
   * ILCR_MILL_REPORT_STATUS_RPT_VW} row. Any component may be null.
   */
  record StatusDates(
      String open1To10,
      String draft1To10,
  ...
  /**
   * Whether a report summary exists for this mill/year and schedule category (Schedule 1 = category
   * {@code "1"}).
   *
   * @param millId the mill id
   * @param year the reporting year
