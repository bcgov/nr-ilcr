...
/**
 * Spring Data JDBC row shape for the two independent track status CODES of one {@code
 * THE.ILCR_MILL_REPORT_STATUS} row (AD-3). The table PK is composite ({@code REPORT_YEAR,
 * ILCR_MILL_ID}); the (mill, year)-filtered read returns 0..1 row, so {@code ILCR_MILL_ID} serves
 * as the read {@code @Id}. Either status code may be NULL (S07 — legacy would NPE on a NULL
 * silviculture code; the service tolerates it). {@link MillContextRepository} maps it to {@link
 * MillContextRepository.TrackCodes}.
 *
