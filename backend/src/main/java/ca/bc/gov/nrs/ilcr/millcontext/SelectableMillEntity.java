 ...
 * so {@code ILCR_MILL_STATUS_CODE} comes from the XREF (not {@code MILL}); the {@code @Table} name
 * is the driving table. Mapped by {@code THE} column name — it never crosses the service boundary:
 ...
 * @param millStatusCode the status code {@code "ACT"}/{@code "CLS"} ({@code
