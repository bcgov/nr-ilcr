 ...
 * (still 3-char-prefixed) per-status display dates (AD-3). On the delivery DB this is a VIEW with
 * no PK/uniqueness guarantee, so {@link MillContextRepository#findStatusDates} reads a list and
 * takes the first row (legacy {@code MillReportStatusDAO} {@code get(0)} parity); {@code
 * ILCR_MILL_ID} serves only as the read {@code @Id}. Any date component may be null. {@link
 * MillContextRepository} maps it to {@link MillContextRepository.StatusDates}.
 *
