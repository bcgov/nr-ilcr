 ...
 *
 * <p>NULLABILITY: {@code description} may be null (a code present in {@code
 * ILCR_MILL_REPORT_STATUS} but absent from the {@code ILCR_MILL_REPORT_STATUS_CODE} lookup) and is
 * then omitted from the JSON under global Jackson {@code non_null} — like {@code MillSummary}'s
 * nullable fields, Story 1.4's banner MUST render a present-{@code code}/ absent-{@code
 * description} track defensively rather than assume the pair is always complete. {@code date} is
 * null when blank/absent (rendered as {@code Not Initiated}).
 *
 * @param code the status code {@code D}/{@code S}/{@code V}/{@code O} ({@code
 *     THE.ILCR_MILL_REPORT_STATUS_CODE.ILCR_MILL_REPORT_STATUS_CODE})
