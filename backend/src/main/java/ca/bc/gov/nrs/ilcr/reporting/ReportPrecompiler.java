...
/**
 * BUILD-TIME ONLY — not a runtime component and never invoked by the app. Run by exec-maven-plugin
 * at the {@code process-classes} phase (where the build JVM is a JDK) to compile every {@code
 * reports/*.jrxml} to a sibling {@code reports/*.jasper}. Baking the compiled expression classes
 * into {@code .jasper} at build time means {@link ReportService} only ever LOADS them at runtime —
 * so the JRE container (no {@code javac}) never compiles a report (the cause of the Print Schedules
 * 500).
 *
 * <p>Uses the same JR7 Jackson load path as the runtime read, so it parses the exact template
 * format the app ships. Arg 0 is the compiled-resources directory (e.g. {@code
 * target/classes/reports}).
 */
public final class ReportPrecompiler {
  ...
  /**
   * Main method to compile Jasper templates at build time.
   *
   * @param args the command line arguments containing the reports directory
  ...
  public static void main(String[] args) throws Exception {
    ...
    // Fail the build if nothing compiled. The runtime has no compile fallback, so a green build
    // that
    // produced zero .jasper files would surface only as a 500 in the JRE container — the exact bug
    // this precompile fixes. A -DskipTests build must not sail past it (the reporting ITs would,
    // but
    // they are a later phase). If templates are added/removed, update the expected count.
    if (compiled == 0) {
      throw new IllegalStateException(
          "No report templates (*.jrxml) found or compiled in "
