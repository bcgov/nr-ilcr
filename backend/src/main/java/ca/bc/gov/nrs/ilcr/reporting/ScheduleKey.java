 ...
 * Declared in the FIXED legacy print order (1 → 5 → 6 → 7A → 7B → 9 → 10 → 11 within the 1..11
 * sequence), so iterating the enum yields the combined PDF's section order (BR-08) without a
 * separate ordering table. Schedule 1 sits first in the legacy sequence, so its constant is
 * declared FIRST. Schedules 2/3/8 and the Mill Information report are out of scope (no enum
 * constant) — a selected-but-unimplemented schedule is skipped-with-a-log by the orchestrator.
 */
