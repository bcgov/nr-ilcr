 ...
 * Declared in the FIXED legacy print order (3 → 5 → 6 → 7A → 7B → 9 → 10 → 11 within the 1..11
 * sequence), so iterating the enum yields the combined PDF's section order (BR-08) without a
 * separate ordering table. Schedule 3 sits before Schedule 5 in the legacy sequence, so its
 * constant is declared FIRST. Schedules 1/2/8 and the Mill Information report are out of scope (no
