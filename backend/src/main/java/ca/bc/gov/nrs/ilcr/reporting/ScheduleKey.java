 ...
 * Declared in the FIXED legacy print order (1 → 2 → 5 → 6 → 7A → 7B → 9 → 10 → 11 within the 1..11
 * sequence), so iterating the enum yields the combined PDF's section order (BR-08) without a
 * separate ordering table. Schedule 1 and 2 sit first in the legacy sequence, so they are declared
 * first. Schedules 3/8 and the Mill Information report are out of scope (no enum constant) — a
