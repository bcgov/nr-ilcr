 ...
 * Declared in the FIXED legacy print order (1 → 2 → 3 → 5 → 6 → 7A → 7B → 8 → 9 → 10 → 11 within
 * the 1..11 sequence), so iterating the enum yields the combined PDF's section order (BR-08)
 * without a separate ordering table. Schedule 1, 2, and 3 sit first in the legacy sequence, so they
 * are declared first. The Mill Information report is out of scope (no enum constant) — a
