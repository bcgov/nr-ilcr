 ...
 * selection (400, FLD-001) or the year already being open (409). Carries a {@code
 * messages.properties} key the {@code GlobalExceptionHandler} resolves to the client-facing
 * ProblemDetail detail (AD-8). The recurring zero-active-mills case (INF-001 + ERR-002 together)
 * uses {@code MultiMessageException}.
 */
public class ReportingYearException extends BusinessException {
  ...
  /**
   * First-time setup with no (or an out-of-range) starting-year selection — nothing is created
   * (FLD-001).
   */
