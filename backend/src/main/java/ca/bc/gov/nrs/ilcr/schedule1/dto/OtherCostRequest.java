...
/**
 * Add/edit request for one itemized Subtotal Other Costs row (AD-12). {@code description} is
 * required (legacy JSF {@code required}) and capped at 30 chars (legacy {@code maxlength="30"});
 * {@code cost} is OPTIONAL — legacy permits a null cost (that is how the WRN-002 empty-cost warning
 * rows arise). The row's volume is not client-supplied: it inherits the shared Other-Costs volume
 * server-side (BR-06). Range messages resolve the LEGACY bundle keys (AD-8) via the wired {@code
 * MessageSource}.
 *
