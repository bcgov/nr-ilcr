 ...
 *
 * <p><strong>A null scalar CLEARS the stored value.</strong> This is a PUT of the whole entered-field
 * set, not a PATCH: the client always sends every field and uses null for "the user emptied this box",
 * so the server writes that null through. Only an absent enclosing BLOCK ({@code lineItems},
 * {@code silviculture}, or one of its {@code EntryAmount} entries) means "not submitted — leave the
 * stored rows alone".
 *
