...
final class Schedule10CheckStatus {
  ...
  record PageOutcome(
      int pageId, int pageNumber, String pageLabel, List<Issue> issues,
  ...
  static Outcome evaluate(Schedule10Response document) {
    ...
    Set<Integer> allowableBec = document.codeLists() == null
        ? Set.of()
    ...
    for (ConstructionPage page : document.pages()) {
      ...
      pages.add(outcome);
      met = met && outcome.issues().isEmpty()
  ...
  static PageOutcome evaluatePage(ConstructionPage page, Set<Integer> allowableBec) {
    ...
    return new PageOutcome(
        page.pageId(), page.pageNumber(), page.pageLabel(), issues, details);
  }
  ...
  static DetailOutcome evaluateRoadDetail(
    ...
    requirePresent(issues, "ballastMaterialCode", prefix + " Additional Stabilizing Type",
  ...
  private static void requirePresent(
      List<Issue> issues, String field, String label, String value) {
  ...
  private static void requireRange(
    ...
    issues.add(new Issue(
        field, label, MSG_RANGE, List.of(format(lower, lowerFormat), format(upper, lowerFormat))));
  }
