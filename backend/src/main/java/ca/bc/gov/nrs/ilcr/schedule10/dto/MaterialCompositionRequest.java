 ...
 *
 * <p><strong>FIVE percentages, not six.</strong> Boulder Area % is removed by business direction and
 * must not appear on the wire. The column remains in the table and is simply never written.
 *
 * <p><strong>There is deliberately no sum-to-100 rule on save.</strong> Legacy has none — the save
 * succeeds regardless of what the five add up to. Check Status is where the total is enforced, and it
 * carries a quirk worth knowing: because the legacy total coerces nulls to zero and is therefore
