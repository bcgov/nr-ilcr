 ...
 * global BEC type-ahead lookup (BR-09). Never crosses the service boundary: {@code
 * Schedule11Service} maps it to {@code dto/BiogeoclimaticOption}, deriving the label from the four
 * raw parts with the same {@code becLabel} concat the served location rows use (legacy {@code
 * getBiogeoSubZoneVariantPase()}, nulls → {@code ""}).
 *
