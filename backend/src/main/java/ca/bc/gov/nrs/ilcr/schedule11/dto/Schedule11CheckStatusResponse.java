 ...
 * {@code 0} is present); {@code requirementsMet} is true iff {@code errors} is empty (zero locations
 * ⇒ vacuously met).
 *
 ...
 * SUC-004 "Status has been checked" text emitted on EVERY invocation (legacy
 * {@code Schedule11MB.checkStatus()} always adds it), and {@code requirementsMetMessage} is the
 * SUC-003 "all requirements met" text present only when {@code requirementsMet}.
 *
 * @param requirementsMet whether every location passes (no errors)
 * @param errors one entry per missing cost, verbatim legacy-composed text
 *     ({@code "location  : {location} - Actual|Planned cost: Value Required"}), in
 *     {@code BASIC_SILVICULTURE_REPORT_ID} order, actual before planned; empty when met
