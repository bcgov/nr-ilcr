 ...
 * {@code subtotal}, {@code netPurchased}, {@code totalCompanyLogging}, {@code totalAverage}, {@code
 * trackStatus}, {@code editable}) is computed server-side and never accepted from a client.
 *
 * <p>The two editable line items are {@code purchasedLogCost} (cost-item 25 — cost entered here,
 * its volume carried from Schedule 3 item 118) and {@code lessLogSales} (cost-item 26 — volume +
 * cost entered). {@code purchasedWoodOverhead}, {@code totalCompanyLogging} volume, and the derived
 * blocks are carried/computed. When the Schedule 3 source data is absent the carried/derived
 * figures are null and (with the app-wide Jackson {@code non_null} inclusion) omitted from the
 * JSON.
 *
