 ...
 * (the Schedule 1 BR-03/BR-04 pre-fill). Both compute the Subtotal Actual Costs the same way, so the
 * line set ({@link #LINES}) and the PO&amp;P/other-acceptable rules ({@link #resolvePop},
...
final class Schedule3Constants {
  ...
  /**
   * A fixed admin-cost line (legacy {@code Constant.REPORT_COST_ITEMS}). Each line's Harvest item id is
   * its identity; {@code popCode} is its PO&amp;P item id (null for the Harvest-only lines).
   */
  record LineSpec(int code, Integer popCode, boolean harvestOnly) {
  }

  static final List<LineSpec> LINES = List.of(
   ...
   */
  static Integer resolvePop(LineSpec spec, Integer harvest, Map<Integer, DetailRow> byCode,
   ...
   */
  static Integer scalingPop(Integer scalingHarvest, BigDecimal popTimberVolume,
