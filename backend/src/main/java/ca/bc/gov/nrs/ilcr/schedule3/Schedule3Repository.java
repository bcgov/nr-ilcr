...
public interface Schedule3Repository extends Repository<ReportSummary, Long> {
   ...
   * (REPORT_YEAR, ILCR_MILL_ID, ILCR_3_ID). The real THE schema has no unique constraint on that
  ...
  @Query(
      ...
      USING (SELECT :millId AS ILCR_MILL_ID, :year AS REPORT_YEAR, '3' AS ILCR_3_ID FROM DUAL) src
         ON (t.ILCR_MILL_ID = src.ILCR_MILL_ID
             AND t.REPORT_YEAR = src.REPORT_YEAR
             AND t.ILCR_3_ID = src.ILCR_3_ID)
       WHEN NOT MATCHED THEN
         INSERT (ILCR_REPORT_SUMMARY_ID, REPORT_YEAR, ILCR_MILL_ID, ILCR_3_ID,
        ...
        .orElseThrow(
            () ->
                new IllegalStateException(
                    "Schedule 3 summary not found immediately after MERGE create"));
  }
