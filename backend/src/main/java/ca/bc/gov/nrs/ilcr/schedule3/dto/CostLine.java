 ...
 * Silviculture Admin (37) PO&amp;P to zero). {@code crown} is derived read-only ({@code
 * CostType.getCrownCost} = harvest &minus; pop, null unless BOTH sides are present) and is
 * ignored/rejected on write (Story 4.2).
 *
 * <p>Scaling Expense (33) is special: its {@code pop} is not stored but derived server-side from
 * the timber-volume ratio, so it carries both {@code pop} and {@code crown}.
 *
 ...
 * @param pop the PO&amp;P-column amount (nullable; 0 for the Harvest-only lines; derived for
 *     Scaling)
...
public record CostLine(
   ...
   */
  public CostLine(Integer costItemCode, Integer harvest, Integer pop, Integer crown) {
    this(costItemCode, harvest, pop, crown, null);
  }
}
