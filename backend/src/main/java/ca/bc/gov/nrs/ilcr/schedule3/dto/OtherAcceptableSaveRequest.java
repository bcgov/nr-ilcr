 ...
 * carries the group's TOT detail {@code id}. The server reconciles by that id: a group with no id is
 * INSERTED as a fresh TOT + PO&amp;P pair, a group whose id matches an existing TOT row is UPDATED in
 * place, and any existing group absent from the request is DELETED. A group carrying an id that is not
 * a TOT row under this summary (stale / concurrently deleted) is rejected with 404 rather than silently
 * re-inserted. (Add/delete are also available as the dedicated POST/DELETE endpoints, which persist
 * immediately.) Validation mirrors {@link OtherAcceptableRequest} and resolves the legacy bundle keys
 * (AD-8).
 *
