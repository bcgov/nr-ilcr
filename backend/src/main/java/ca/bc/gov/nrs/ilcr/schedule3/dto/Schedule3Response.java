 ...
 * Management Administration Costs report. All derived/read-only fields (each line's {@code crown},
 * the timber costs/{@code perUnit}, the four total blocks, {@code trackStatus}, {@code editable},
 * the counts) are computed server-side and are ignored/rejected on write (Story 4.2). {@code
 * revisionCount} is the optimistic-lock token echoed for the Story 4.2 PUT. {@code message} is null
 * on GET (Jackson omits it) and carries the success message on the mutation echo (AD-8/EQ-M3).
 *
 ...
 * @param subtotalActualCosts the Subtotal Actual Costs (sum of the fixed lines + Subtotal Other
 *     Costs)
 * @param includedUnacceptableCosts the Included Unacceptable Costs (item-38 rows + Annual Rents
 *     harvest)
 ...
 * @param unacceptableCount the Included Unacceptable Costs sub-page row count (CNT-001; +1 for
 *     Annual Rents)
 * @param warnings advisory, non-blocking messages carried on a mutation echo (empty on GET); the
 *     BR-09 Crown Timber push outcome (WRN-001 applied / WRN-002 not-opened) rides here on the
 *     Story 4.2 PUT
...
public record Schedule3Response(
  ...
  /**
   * A copy of this document carrying the given advisory warnings (e.g. the BR-09 crown-push
   * outcome).
   */
