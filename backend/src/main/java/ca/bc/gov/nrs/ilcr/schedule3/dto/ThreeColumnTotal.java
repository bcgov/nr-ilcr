 ...
 * Included Unacceptable Costs, and Total Costs. All fields are computed server-side read-only and
 * are never accepted from a client. {@code crown} follows {@code CostType.getCrownCost} (harvest
 * &minus; pop); the subtotals seed at zero (legacy {@code CoreUtil.sumCostType}) so they are always
 * present, while Included Unacceptable forces {@code pop} to 0 so its {@code crown} equals its
