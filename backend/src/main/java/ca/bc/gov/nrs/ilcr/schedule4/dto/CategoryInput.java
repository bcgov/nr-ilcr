...
/**
 * One entered transportation-category amount in a {@link Schedule4LocationRequest} (Story 4.2,
 * AD-12 write contract). {@code code} is the legacy {@code ILCR_REPORT_COST_ITEM_ID}; the server
 * derives FIXED vs DISTANCE from the code and ignores any {@code distance} on a fixed code.
 *
 ...
 * BR-04 (distance ⇄ volume/cost, distance codes only) is the class-level {@link
 * DistanceCategoryComplete} cross-field constraint. All amounts are nullable — a blank category
 * (name-only location, S08) and a cleared field (persists null) are legal.
 *
...
public record CategoryInput(
    ...
    @DecimalMin(value = "0", message = "{volumeValidatorErrorMsg}") @DecimalMax(value = "9999999", message = "{volumeValidatorErrorMsg}") BigDecimal volume,
