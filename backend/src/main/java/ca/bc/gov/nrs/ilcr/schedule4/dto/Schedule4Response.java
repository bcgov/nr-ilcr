 ...
 * response, frozen for all Schedule 4 stories. Unlike Schedules 1–3 (flat aggregates on {@code
 * ILCR_REPORT_SUMMARY}), Schedule 4 is a multi-location structure: each {@link Location} is a
 ...
 * server-side (AD-5), never client-supplied. A non-Draft mill still lists its locations ({@code
 * editable:false}). A valid, active mill/year with no {@code TRANSPORTATION_REPORT} rows for
 ...
 * omits it) and carrying the resolved success {@link MessageInfo} on the Story 4.2 save echo.
 * Adding it is an additive, backward-compatible extension of the frozen read contract (same pattern
 * as {@code Schedule1Response}/{@code Schedule2Response}).
 */
