 ...
 * Schedule 5 master record. Each row is one logging camp keyed by {@code ILCR_MILL_ID} + {@code
 * REPORT_YEAR} + {@code ILCR_CATEGORY_ID = '5'}; its twelve category amounts hang off it as keyed
 * rows in {@code ILCR_COST_REPORT_DETAIL} (see {@link CostReportDetailEntity}).
 *
 * <p>Types are the delivery column types, not legacy's Hibernate mappings (Story 7.1 Task 1 gate
 * (i)): {@code DISTANCE_TO_OPERATING_AREA} is {@code NUMBER(8,2)} and is read as {@code BigDecimal}
 * — legacy mapped it to a {@code Double} and then did {@code new BigDecimal(double)}, which leaks
 * the binary expansion of a value the column had already fixed at scale 2 ({@code 0.1} becomes
 * {@code 0.1000000000000000055…}); a JSF {@code DecimalFormat} hid that, a JSON API would not.
 * {@code ISOLATED_CAMP_IND} stays the raw {@code Y}/{@code N} string here and becomes a Boolean in
 * the service.
 *
