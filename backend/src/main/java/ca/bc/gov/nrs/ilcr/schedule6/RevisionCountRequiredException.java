 ...
 * rejects a null {@code revisionCount} as a 400 — but the service unboxes the token to an
 * {@code int}, so this makes the failure a clean 400 instead of an NPE-driven 500 for any future
 * caller that bypasses the group (code review 2026-08-04, defence in depth).
 *
 ...
 * token is a malformed request, never a stale one (the Story 2.1 review lesson — a coerced 409 tells
 * the user to reload when the real fix is to send the token).
 */
