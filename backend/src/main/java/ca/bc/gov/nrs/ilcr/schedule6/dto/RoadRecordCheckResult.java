...
/**
 * One road record's Check Status result (S09–S11, S20, S21). The message text keys on
 * {@code rowCounter} — the 1-based ordinal in {@code ROAD_MAINTENANCE_REPORT_ID} order,
 * placeholders excluded (legacy {@code Schedule6MB} composes from {@code rowCounter}, assigned by
 * DB read order; the Gherkin pins {@code "Road : 1 - …"}) — while {@code recordId} travels for UI
 * correlation. The ordinal equals the record's 1-based index in the GET document's
 * {@code roadRecords[]} (same ordering, same placeholder exclusion).
 *
...
public record RoadRecordCheckResult(
    ...
    int rowCounter,
    boolean met,
    MessageInfo metMessage,
    List<FieldIssue> issues) {

  /**
   * One missing-field finding: {@code field} names the request field the user must supply
   * ({@code areaType}, {@code tflNumber}, {@code supplyBlock}, or {@code cost}); {@code message}
   * carries the composed verbatim line (e.g. {@code "Road : 1 - TFL Number : Value Required"})
   * under the shared {@code missingRequiredFieldMsg} key.
   *
