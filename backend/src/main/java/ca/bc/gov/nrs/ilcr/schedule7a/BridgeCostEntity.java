...
/**
 * Spring Data JDBC row shape for one Schedule 7A bridge cost detail (AD-3): a
 * {@code THE.ILCR_COST_REPORT_DETAIL} row belonging to a bridge, keyed by {@code BRIDGE_REPORT_ID}
 * plus the fixed Schedule 7 cost-item id ({@code ILCR_REPORT_COST_ITEM_ID} ∈
 * {70,71,72,73,74,75,76,79,80,81} — legacy {@code REPORT_COST_ITEMS.Schedule7_*}).
 * {@code ILCR_REPORT_SUMMARY_ID} is NULL for this list schedule. {@code VOLUME}/{@code COMMENTS}
 * are unused by Schedule 7A. Never crosses the service boundary — {@link Schedule7aService} routes
 * each cost row to its bridge field by item id.
 *
