 ...
 */
public record Schedule7aResponse(
    long millId,
    int year,
    String trackStatus,
    boolean editable,
    List<Bridge> bridges,
    BridgeCodeLists codeLists,
    MessageInfo message) {

  /** A copy carrying the given success message (for a POST/PUT/DELETE echo, AD-8). */
  public Schedule7aResponse withMessage(MessageInfo message) {
    return new Schedule7aResponse(millId, year, trackStatus, editable, bridges, codeLists, message);
  }
}
