 ...
 * resolve to a real reference row. Backend enforcement of the legacy autocomplete's "a value must be
 * selected from the list" — a bad code must be rejected with a 400, not silently persisted (a rate
 * row whose cost item is neither an addition nor a deduction would otherwise vanish from
