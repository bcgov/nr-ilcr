 ...
 * their server-side sum (AD-6). {@code actualHarvested} is {@code coniferousVolume +
 * deciduousVolume}. {@code skidTypeCode} is {@code ILCR_SKID_TYPE_CODE} and {@code
 * skidTypeDescription} its resolved label (§Decision 3). {@code additionsTotal}/{@code
 * deductionsTotal} are the sums of the respective rate rows' costing rates and {@code finalRate =
 * originalRate + additionsTotal − deductionsTotal} — all computed server-side, read-only, never
 * accepted on write (AD-5). {@code additionCount}/ {@code deductionCount} mirror the list sizes.
 * {@code uphillDirection}/{@code waterDumpDestination} are the legacy Y/N indicators as booleans.
 * Money/volume BigDecimals are normalized to natural form.
 */
