 ...
 * {@code "ISSUES"}. {@code messages} carries the schedule-level banner (the SUC-003 all-met message
 * on MET, empty on ISSUES). {@code pages} is the per-page → per-sample → per-field breakdown.
 * Serves both the all-pages sweep and the single-page scope (the single-page result carries one
 * page entry).
 */
