 ...
 * licensee's data. Absent and foreign are indistinguishable from the outside. On an edit this is the
 * 404 half of the AR11 404-vs-409 disambiguation — a guarded {@code UPDATE} affecting zero rows means
 * either an unknown/foreign id (this) or a stale token ({@code StaleRevisionException}), and only a
 * scoped existence probe can tell them apart.
 *
 * <p>Legacy has no message for this case — the screen only ever passed ids it had just rendered — so
 * the key is new (the {@code campNotFoundErrorMsg} precedent). Maps to 404.
 */
