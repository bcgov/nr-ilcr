 ...
 * (AD-1). The server is the sole authority for {@code editable} (AD-9/S30): derived from the caller's
 * {@code EDIT_SCHEDULE} action and, in the service, the Draft track status; the frontend never
 * computes it.
 *
 ...
 *
 * <p>The write responses echo the recomputed document with the AD-8 success message
 * ({@code dataSavedSuccesfullyInfoMsg} on add/edit, {@code dataDeletedSuccesfullyInfoMsg} on delete),
 * resolved to verbatim text here so the service stays message-free. {@code Authentication} is a method
 * parameter, never {@code SecurityContextHolder}, which is empty on Boot-4 MockMvc paths.
 */
@RestController
public class Schedule9Controller implements Schedule9Api {
  ...
  public ResponseEntity<Schedule9Response> addRecord(
      String millId, String year, ContractualWorkRecordRequest request,
      Authentication authentication) {
    MillYearContext context = millContextService.validateMillYearActive(millId, year);
    Schedule9Response doc = schedule9Service.addRecord(
  ...
  public ResponseEntity<Schedule9Response> updateRecord(
      int id, String millId, String year, ContractualWorkRecordRequest request,
      Authentication authentication) {
    MillYearContext context = millContextService.validateMillYearActive(millId, year);
    Schedule9Response doc = schedule9Service.updateRecord(
        context.millId(), context.year(), id, request,
        permissions.hasPermission(authentication, EDIT_SCHEDULE), authentication.getName());
    return ResponseEntity.ok(doc.withMessage(message(MSG_SAVED)));
  }
  ...
  public ResponseEntity<Schedule9Response> deleteRecord(
    ...
    MillYearContext context = millContextService.validateMillYearActive(millId, year);
    Schedule9Response doc = schedule9Service.deleteRecord(
        context.millId(), context.year(), id,
  ...
  public ResponseEntity<Schedule9CheckStatusResponse> checkStatus(
    ...
    MillYearContext context = millContextService.validateMillYearActive(millId, year);
    return ResponseEntity.ok(
        schedule9Service.checkStatus(context.millId(), context.year()));
  }
