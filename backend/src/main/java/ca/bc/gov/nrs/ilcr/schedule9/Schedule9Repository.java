...
public interface Schedule9Repository extends Repository<ContractualWorkReportEntity, Integer> {
   ...
   * the write {@code @Transactional}. A mill/year with no status row locks nothing and returns empty,
   * which the gate already answers as 409.
   */
  @Query("""
      SELECT ILCR_MILL_REPORT_STATUS_CODE
        FROM THE.ILCR_MILL_REPORT_STATUS
  ...
  // UPDATE and DELETE is scoped to (id, ILCR_MILL_ID, REPORT_YEAR, ILCR_CATEGORY_ID='9') as the IDOR
  // guard. EVERY INSERT supplies REVISION_COUNT plus both audit pairs: on CONTRACTUAL_WORK_REPORT all
  ...
  // (T1 gate (i): the ILCR_CRDA_B_I_U-style trigger resolves the parent, so a cost line written first
   ...
   * {@code @SequenceGenerator(sequenceName = "ILCR_REPORT_COMMON_SEQ")} — the same sequence Schedule
   * 5's camps draw from). Fetched separately because {@code @Modifying} cannot return generated keys.
   */
   ...
   * audit stamps). Legacy {@code Schedule9DO.addContractualWorkReport} sets {@code REVISION_COUNT = 0}
   * and stamps {@code ENTRY_*}/{@code UPDATE_*} from the acting user + now.
   */
  @Modifying
  @Query("""
  ...
  void insertRecord(
      ...
      @Param("contractorId") String contractorId, @Param("sideSlopePct") Integer sideSlopePct,
      @Param("numberOfUnits") BigDecimal numberOfUnits, @Param("unitCode") String unitCode,
      @Param("unitDescription") String unitDescription, @Param("sourceCode") String sourceCode,
      @Param("sourceDescription") String sourceDescription, @Param("becZoneCode") String becZoneCode,
   ...
   * summary-less, so there is no schedule-level revision to key on). Bumps {@code REVISION_COUNT} and
   * stamps {@code UPDATE_*} ONLY when the stored revision still matches AND the row belongs to this
   * mill/year/category. {@code ENTRY_*} is untouched, so audit provenance survives an edit.
   *
   * @return rows affected — {@code 1} on success; {@code 0} when the id is absent/foreign (→ 404) OR
   *     the revision is stale (→ 409). {@link #countRecord} disambiguates.
   */
  @Modifying
  @Query("""
  ...
  int updateRecord(
      @Param("id") int id, @Param("millId") long millId, @Param("year") int year,
      @Param("expectedRevision") int expectedRevision, @Param("contractorId") String contractorId,
      @Param("sideSlopePct") Integer sideSlopePct, @Param("numberOfUnits") BigDecimal numberOfUnits,
      @Param("unitCode") String unitCode, @Param("unitDescription") String unitDescription,
      @Param("sourceCode") String sourceCode, @Param("sourceDescription") String sourceDescription,
      @Param("becZoneCode") String becZoneCode, @Param("comments") String comments,
   ...
   * id) rather than 409 (stale token). Returns {@code 0} or {@code 1}; the PK makes more impossible.
   */
  @Query("""
      SELECT COUNT(*)
  ...
  /**
   * Insert the record's single cost line — the Contractual Item (108–114), the cost, and the "Other"
   * item free text ({@code ITEM_DESCRIPTION}). {@code REVISION_COUNT = 0} plus both audit pairs
   * (legacy stamps the child too). Only {@code CONTRACTUAL_WORK_REPORT_ID} is populated among the
   * parent FKs, which the shared-table insert trigger requires (exactly one parent FK).
   */
  @Modifying
  @Query("""
      ...
      """)
  void insertCostLine(
      @Param("id") int id, @Param("recordId") int recordId, @Param("itemCode") int itemCode,
      @Param("cost") Integer cost, @Param("itemDescription") String itemDescription,
      @Param("user") String user);
   ...
   * nothing in delivery makes {@code (CONTRACTUAL_WORK_REPORT_ID)} unique on this shared table, read
   * and write agree on which line is the record's — the read's {@code toMap} first-wins dedup keys on
   * the same lowest id. {@code REVISION_COUNT} is deliberately NOT bumped (legacy moves only {@code
   * UPDATE_*} on the child); a null cost/description writes {@code NULL}.
   *
  ...
  /**
   * Delete the record's cost line(s) — runs BEFORE the master (child-first; the parent FK is
   * {@code ON DELETE NO ACTION} in delivery, so deleting the master first would raise ORA-02292).
   * Scoped by an {@code EXISTS} against the mill/year/category-{@code '9'} parent (the IDOR guard),
   * so the method is safe in isolation and not only by call-order convention.
   *
  ...
  /**
   * Delete one master, scoped to mill/year/category (the IDOR guard). Call only after
   * {@link #deleteCostLinesForRecord}.
   *
  ...
  /**
   * Whether a code value resolves to a live {@code ILCR_UNIT_CODE} row (force-selection FLD-005). Not
   * year-scoped — the 9.1 read path resolves unit descriptions with an unfiltered join, so a write
   * must accept exactly the codes the read can name.
   */
