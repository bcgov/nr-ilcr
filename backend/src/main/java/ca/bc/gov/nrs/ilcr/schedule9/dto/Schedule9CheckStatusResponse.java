 ...
 * transition and no mutation. Reproduces {@code Schedule9CheckStatus.validateSchedule}: every
 * stored record is walked in served (id) order and each of its EIGHT fields tested in the exact
 * legacy order (Company ID, Contractual Item, Side Slope %, Number of Units, Unit Type,
 * Biogeoclimatic Zone, Cost$, Source). The three "Other" descriptions are deliberately NOT tested —
 * the per-field {@code isXxxDescriptionValid} methods exist in legacy but {@code validateSchedule}
 * never calls them.
 *
 ...
 * = "Value Required"); a Side Slope of exactly 100 saves (≤100) but is flagged here (the Check
 * bound is 0..99). A record with all eight satisfied contributes no line.
 *
 ...
 * @param requirementsMetMessage SUC-002 {@code scheduleRequirementsMetMsg} ({@code "All
 *     requirements for this schedule have been met"}) when all pass, else null (omitted)
 */
