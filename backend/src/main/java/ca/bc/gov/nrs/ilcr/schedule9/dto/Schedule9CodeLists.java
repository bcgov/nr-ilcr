 ...
 * without a second call. Each option is a {@code (code, description)} pair, the same shape the record
 * fields already serve, so the page can match a record's stored code to its option.
 *
 ...
 *
 * <p><strong>Not year-scoped.</strong> Unlike Schedule 7A/7B (which filter to codes effective for the
 * reporting year via a {@code LookupCache} port), these lists serve every reference row, matching the
 * 9.1 read joins, which resolve descriptions unscoped. The Contractual Item catalogue is the fixed
 * category-{@code '9'} set (108–114, BR-09); the unit/BEC/source reference tables are small and
 * stable. Year-scoping the unit list (the only reference table here with effective/expiry columns) is
 * a possible later refinement, recorded rather than done, to keep this addition minimal.
 *
