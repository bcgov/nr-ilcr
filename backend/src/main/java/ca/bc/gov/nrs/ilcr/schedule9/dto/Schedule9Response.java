 ...
 * {@code '9'}) whose Contractual Item + Cost live as a KEYED row in the shared {@code
 * THE.ILCR_COST_REPORT_DETAIL}, joined by {@code CONTRACTUAL_WORK_REPORT_ID} (legacy {@code
 * ILCRCostReportDetail.@JoinColumn("CONTRACTUAL_WORK_REPORT_ID")}); the cost line's {@code
 * ILCR_REPORT_COST_ITEM_ID} is the contractual item (108–114, BR-09). Like Schedules 4/5/6 this
 * schedule is summary-less — there is NO category-{@code '9'} {@code ILCR_REPORT_SUMMARY} row, so
 * the read guards on {@code validateMillYearActive}, not {@code validateScheduleViewable("9")}
 ...
 * lock token (writes are Story 9.2). A valid, ACTIVE mill/year with no records returns {@code
 * records: []}, never a 404. {@code message} is the AD-8 success echo: null on a GET read (Jackson
 * {@code non_null} omits it), carried on the 9.2 save echo via {@link #withMessage}.
 */
