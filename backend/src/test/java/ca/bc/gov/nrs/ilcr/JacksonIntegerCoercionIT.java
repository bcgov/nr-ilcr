...
/**
 * Cross-schedule regression pin for the APP-WIDE {@code spring.jackson.deserialization.accept-float-as-int:
 * false} in {@code application.yml}.
 *
 ...
 * Cross-cutting — identical behaviour in Schedule 1"). Turning the Jackson feature off in the shared
 * {@code application.yml} closed it for every schedule at once instead of leaving Schedule 5 the only
 * safe one — but it is a compatibility change to every endpoint in the application, and the only
 * evidence offered for it was that the whole suite stayed green. Code review (PR #242) asked for the
 * blast radius to be MEASURED on schedules other than 5 rather than inferred. That is this class: it
 * asserts, on endpoints owned by other epics, that a fractional value posted into an {@code Integer}
 * field is a clean 400 and not a 200-with-a-different-number.
 *
 ...
 * Draft.</strong> That is a fail-safe, not an accident. Body binding happens during handler-argument
 * resolution, so a rejected value never reaches a controller, a service, or the database — but if the
 * pin ever regressed, a Draft mill/year would let these requests write real rows before the assertion
 * failed. Against a Submitted one the regression surfaces as a 409 from the Draft gate and still
 * touches nothing.
 */
...
class JacksonIntegerCoercionIT extends AbstractOracleIT {
  ...
  /**
   * The verbatim legacy {@code costConverterErrorMsg} (FLD-004). A refused float reaches
   * {@code GlobalExceptionHandler.handleNotReadable}, whose cause message names {@code
   * java.lang.Integer}, so it resolves the COST converter text rather than the generic "request body
   * is invalid" — asserted rather than assumed, because that branch is what makes the rejection
   * legible to a licensee instead of merely correct.
   */
  ...
  void schedule1OtherCostRejectsFractionalCost() throws Exception {
    ...
    mockMvc.perform(post("/api/v1/schedule1/other-costs").with(csrf())
  ...
  void schedule6RoadRecordRejectsFractionalCost() throws Exception {
    // ca.bc.gov.nrs.ilcr.schedule6.dto.RoadRecordRequest#cost is an Integer; its sibling volume is a
    ...
    mockMvc.perform(post("/api/v1/schedule6/records").with(csrf())
            .param("millId", MILL).param("year", YEAR)
            .contentType(MediaType.APPLICATION_JSON)
  ...
  @Test
  @DisplayName("a WHOLE number written as 1234.0 is rejected too — the rule is the type, not the value")
