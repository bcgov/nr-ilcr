 ...
 * separate ordering table), so this pins the FIXED legacy sequence: Schedule 2 (Story 20.6) is
 * declared FIRST — ahead of Schedule 5 — and Schedule 10 (Story 20.4) sits between Schedule 9 and
 * Schedule 11. A combined print always emits each in that fixed position relative to the other
 * selected sections. No Spring context or database.
 */
@DisplayName("ScheduleKey — fixed legacy section order (BR-08)")
class ScheduleKeyTest {

  @Test
  @DisplayName("values() are in the fixed legacy print order (2 first, 10 between 9 and 11)")
