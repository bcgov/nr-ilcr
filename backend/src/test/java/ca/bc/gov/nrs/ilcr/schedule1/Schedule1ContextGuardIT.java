 ...
 *
 * <p>These acceptance tests assert the mill/year CONTEXT GUARD contract for {@code GET
 * /api/v1/schedule1}. They reference no unbuilt production type (URL + ProblemDetail only), so this
 * file COMPILES today; it is class-{@code @Disabled} as the red-phase gate. Runs with security OFF
 * (mock principal = ILCR_SUBMITTER, which holds VIEW_SCHEDULE) so these isolate the context guards,
 * not authz — 403 is proven separately in {@link Schedule1AuthorizationIT}.
 *
 * <p>RED expectation before implementation: with no {@code Schedule1Controller} the calls 404/500
 * instead of the asserted statuses — that IS red. The S19 param cases may specifically surface as
 * 500 (today's catch-all) until {@code GlobalExceptionHandler} adds the 400 handlers — also valid
 * red.
 *
 * <p>GREEN = correct status + {@code application/problem+json} + verbatim legacy message text (from
 * the story's MESSAGE BUNDLE; keys preserved incl. trailing space on ERR-002/003 values).
 *
 * <p>ACTIVATION (dev-story Task 9): create the test schema/seed (Task 8), the millcontext guard
 * chain (Task 6), and the controller skeleton (Task 7); set {@code ilcr.security.enabled=false} for
 * this class; then remove {@code @Disabled}.
 */
...
class Schedule1ContextGuardIT extends AbstractOracleIT {
  ...
  // Seeded by the shared test snapshot (Task 8): mill 514 – AAA Milling, year 2021, Sch1 track = D
  // (Draft).
  ...
  void missingMillId_returns400() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("year", String.valueOf(SEEDED_YEAR)))
  ...
  void missingYear_returns400() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("millId", String.valueOf(SEEDED_MILL)))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(PROBLEM_JSON));
  }
  ...
  void nonNumericMillId_returns400() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("millId", "abc").param("year", String.valueOf(SEEDED_YEAR)))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(PROBLEM_JSON));
  }
  ...
  void unknownMill_returns404() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("millId", "999999").param("year", String.valueOf(SEEDED_YEAR)))
  ...
  void millClosedForYear_returns409_verbatimMessage() throws Exception {
    ...
    mockMvc
        .perform(get(ENDPOINT).param("millId", "516").param("year", String.valueOf(SEEDED_YEAR)))
            ...
            jsonPath(
                ...
                is(
                    "This Mill is not active for the current Reporting Year. "
                        + "Please select another mill from the Home Page."))); // ERR-002 /
    // millNotActiveForCurrentYearMsg
  }
  ...
  void validContext_passesGuardChain() throws Exception {
    ...
    // NOTE: full document assertions (lineItems/trackStatus/editable/otherCosts/perUnit) belong to
    // Story 1.2.
  }
}
