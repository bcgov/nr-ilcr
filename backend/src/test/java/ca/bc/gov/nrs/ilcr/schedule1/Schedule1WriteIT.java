 ...
 * holds EDIT_SCHEDULE — this isolates the persistence / write-gate / concurrency behaviour from
 * authz (the 403 case lives in {@link Schedule1WriteAuthorizationIT}, which runs with security ON).
 * Request bodies are raw JSON text blocks so this file COMPILES before the {@code Schedule1Request}
 * DTO exists — the pinned wire contract is asserted via jsonPath + direct DB reads, never by
 * referencing an unbuilt production type.
 *
 ...
 * whole run): mill 518 / summary 1018 (save + ignore-derived; itemized item-19 Row A/B ids
 * 5020/5021), mill 519 / summary 1019 (DELETE, S13), mill 520 / summary 1020 (optimistic lock,
 * AC7), mill 517 / summary 1017 (Submitted — non-Draft write gate, S22-write). Only ONE test
 * persists to 518, and it reads the revision at runtime, so cases stay order-independent.
 *
 * <p>ACTIVATION (dev-story): implement Tasks 1–6, then remove {@code @Disabled} and green each
 * case.
 */
...
class Schedule1WriteIT extends AbstractOracleIT {
  ...
  // NOTE (Task 1 open item): which fixed fields are 7-digit vs 8-digit volume validators is
  // resolved
  // from legacy schedule1.xhtml during dev-story. FLD-002 targets a line-item volume; FLD-003
  // targets
  ...
  @Test
  @DisplayName(
      "S07 — non-numeric volume -> 400 FLD-005 (volumeConverterErrorMsg), nothing persisted")
  ...
  void delete_nonDraft_returns409() throws Exception {
    mockMvc
        .perform(delete(ENDPOINT).param("millId", "517").param("year", "2021"))
        .andExpect(status().isConflict())
        .andExpect(
            jsonPath("$.detail", is("This schedule cannot be edited in its current status.")));
    assertEquals(
        1,
        JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, SUMMARY, "ILCR_REPORT_SUMMARY_ID = 1017"),
  ...
  void delete_removesWholeSchedule() throws Exception {
    mockMvc
        .perform(delete(ENDPOINT).param("millId", "519").param("year", "2021"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message.text", is("Data deleted successfully")));
    assertEquals(
        0,
        JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, SUMMARY, "ILCR_REPORT_SUMMARY_ID = 1019"),
        "summary row must be gone (BR-08)");
    assertEquals(
        0,
  ...
  // ----------
  // Implemented in Schedule1WriteFailureIT (its own @MockitoSpyBean context, so fault injection
  // does
