 ...
 * rendered bounds are the contract. It also fingerprints every mutable column before and after,
 * since "mutates nothing" is a claim that a passing status assertion alone would never catch.
 */
...
class Schedule10CheckStatusIT extends AbstractOracleIT {
  ...
  /**
   * Every mutable column of every Schedule 10 row for a mill, across ALL THREE tables the write
   * path touches, including both audit pairs on each.
   *
   * <p>Code review 2026-08-18 found this helper's javadoc overclaiming: the road-detail leg
   * captured neither timestamp, and {@code ILCR_COST_REPORT_DETAIL} was not fingerprinted at all —
   * so a check-status run that re-stamped a detail or touched a cost row would have gone unnoticed.
   * The cost leg is joined up through the detail to the page so the mill filter still applies.
   */
  ...
  void incompleteScheduleReportsComposedIssues() throws Exception {
    ...
    // One chain over the same subject, with each link's reasoning kept beside it. Every anyMatch
    // and
    ...
    assertThat(texts)
        ...
        .anyMatch(
            text -> text.endsWith("Material Type Total (%): Total value must be equal to 100."))
        // The Road Name and Sub Zone rules carry the PAGE label only, with no road label — so on
        // page 8951, which holds two roads, their lines would be indistinguishable. That is
        // legacy's
        ...
        .anyMatch(text -> text.endsWith(" Additional Stabilizing: Length (km): Value Required"))
        .anyMatch(text -> text.endsWith(" Additional Stabilizing: Actual Cost ($): Value Required"))
        ...
        .noneMatch(text -> text.endsWith(" Additional Stabilizing Type: Value Required"));
  ...
  void submittedScheduleCanStillBeChecked() throws Exception {
    ...
    //
    // The outcome VALUE is asserted, not merely its presence: exists() reads like a result
    // assertion
