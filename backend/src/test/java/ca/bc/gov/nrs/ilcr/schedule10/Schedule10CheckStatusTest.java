...
class Schedule10CheckStatusTest {
  ...
  private static RoadDetail cleanDetail() {
    return detail(cleanSubGrade(), stabilizingNotRequired(), material(10, 20, 40, 20, 10, 100), 25);
  }
  ...
  class Passing {
    ...
    void completeScheduleIsMet() {
      PageOutcome outcome =
          Schedule10CheckStatus.evaluatePage(page(List.of(cleanDetail())), ALLOWABLE);
  ...
  class Quirks {
    ...
    void blankMaterialStillReportsTotal() {
      ...
      RoadDetail blankMaterial =
          detail(
              cleanSubGrade(),
              stabilizingNotRequired(),
              material(null, null, null, null, null, 0),
              25);
      ...
      assertThat(total.label())
          .isEqualTo(PAGE_LABEL + ", Road #1, Mainline A" + " Material Type Total (%)");
    }
    ...
    void roadNameAndSubZoneUsePageLabelOnly() {
      ...
      // name promised Sub Zone too, and its fixture supplied a non-blank subzone so no subzone
      // issue
      // could be produced at all (code review 2026-08-18). A blank subzone is unreachable from
      // stored
      // data — the column is NOT NULL — but it IS reachable at this seam, which is the whole point
      // of
      // pinning the rule here.
      BecClassification blankSubzone =
          new BecClassification(BEC_ID, "ICH", "  ", "1", null, "ICH1");
      RoadDetail nameless =
          new RoadDetail(
              8910,
              1,
      ...
      assertThat(issueFor(outcome.issues(), "subzone").label()).isEqualTo(PAGE_LABEL + " Sub Zone");
    ...
    void emissionOrderIsContractual() {
      ...
      // assertion used order-insensitive matchers, so reordering any of the 36 road-detail rules
      // left
      // the whole suite green (code review 2026-08-18). Pinned as an exact sequence.
      Stabilizing crushedEmpty =
          new Stabilizing("C", null, null, null, null, null, null, null, null, null, null);
      RoadDetail bare =
          new RoadDetail(
              8910,
              1,
      ...
      DetailOutcome outcome = Schedule10CheckStatus.evaluateRoadDetail(bare, PAGE_LABEL, ALLOWABLE);
    ...
    void endHaulAndOverlandAreNotChecked() {
      ...
      // nothing at all (Sonar Bug, 2026-08-18). This states the actual claim: four out-of-range
      // haul
      ...
      assertThat(
              Schedule10CheckStatus.evaluateRoadDetail(alsoBadSideSlope, PAGE_LABEL, ALLOWABLE)
                  .issues())
          .extracting(Issue::field)
          .containsExactly("sideSlopePct");
    }

    @Test
    @DisplayName(
        "stabilizing transfers are floored at zero here even though the form allows negatives")
    void stabilizingTransfersAreFlooredAtZero() {
      ...
      RoadDetail withNegativeTransfers =
          detail(cleanSubGrade(), crushed, material(10, 20, 40, 20, 10, 100), 25);
  ...
  class BallastGate {
    ...
    void crushedRequiresEverything() {
      Stabilizing empty =
          new Stabilizing("C", null, null, null, null, null, null, null, null, null, null);
      RoadDetail detail = detail(cleanSubGrade(), empty, material(10, 20, 40, 20, 10, 100), 25);

      DetailOutcome outcome =
          Schedule10CheckStatus.evaluateRoadDetail(detail, PAGE_LABEL, ALLOWABLE);

      // containsExactly, not contains: the sub-grade and material figures are all clean here, so
      // the
      ...
      // satisfied — so it never pinned the material-type rule to its own message (flagged by both
      // the
    ...
    void notRequiredSkipsTheGatedRules() {
      Stabilizing empty =
          new Stabilizing("N", null, null, null, null, null, null, null, null, null, null);
      RoadDetail detail = detail(cleanSubGrade(), empty, material(10, 20, 40, 20, 10, 100), 25);
      ...
      Stabilizing sameButCrushed =
          new Stabilizing("C", null, null, null, null, null, null, null, null, null, null);
      assertThat(
              Schedule10CheckStatus.evaluateRoadDetail(
                      detail(
                          cleanSubGrade(), sameButCrushed, material(10, 20, 40, 20, 10, 100), 25),
                      PAGE_LABEL,
                      ALLOWABLE)
                  .issues())
  ...
  class Bec {
    ...
    @Test
    @DisplayName(
        "a classification outside the allowable list is reported with legacy's own message")
  ...
  class BoundFormatting {
    ...
    void percentageBounds() {
      RoadDetail overSlope =
          detail(cleanSubGrade(), stabilizingNotRequired(), material(10, 20, 40, 20, 10, 100), 101);
    ...
    void moneyBounds() {
      SubGrade overCost =
          new SubGrade(
              ...
              null);
      RoadDetail detail =
          detail(overCost, stabilizingNotRequired(), material(10, 20, 40, 20, 10, 100), 25);
    ...
    void lengthBounds() {
      SubGrade tooLong =
          new SubGrade(
              ...
              null);
      RoadDetail detail =
          detail(tooLong, stabilizingNotRequired(), material(10, 20, 40, 20, 10, 100), 25);
