...
class Schedule10RoadDetailWriteIT extends AbstractOracleIT {
   ...
   * too-long name comes back as a bare 400 from {@code newPage}, which reads like a broken write path
   * and cost a full suite run to diagnose (2026-08-18).
   */
  ...
  void ballastDeferredKeepsFiguresButForcesMaterial() throws Exception {
    // The documented asymmetric branch had no test at all: only N and C appeared anywhere, so folding
    // D into the N branch — silently zeroing four real dimensions and two real costs — was invisible
  ...
  void unknownDetailCodesAreRejected() throws Exception {
    ...
    // offered codes, so deleting the road-lifetime, RSMR or ballast-method guard left the suite green
    ...
    mockMvc.perform(post(base).param("millId", MILL).param("year", YEAR)
            .contentType(MediaType.APPLICATION_JSON).with(csrf())
            .content("""
                ...
                """))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.detail", containsString("A valid value must be selected from the list.")));
    ...
    assertThat(jdbc.queryForObject(
        "SELECT COUNT(*) FROM THE.ROAD_CONSTRUCTION_REPRT_DTL"
            + " WHERE ROAD_CONSTRUCTION_REPRT_ID = ?", Integer.class, pageId)).isZero();
  }
  ...
  void staleRevisionOnRoadDetailIsConflict() throws Exception {
    // The road-detail optimistic lock had NO test at all — the only stale-revision test in the story
    ...
    mockMvc.perform(put(PAGES + "/" + pageId + "/road-details/" + detailId)
    ...
    assertThat(jdbc.queryForObject(
        "SELECT ROAD_NAME FROM THE.ROAD_CONSTRUCTION_REPRT_DTL"
            + " WHERE ROAD_CONSTRUCTION_REPRT_DTL_ID = ?", String.class, detailId))
        .isEqualTo("Stale Road");

    // The other half of the disambiguation: an id that does not exist is a 404, not a 409.
    mockMvc.perform(put(PAGES + "/" + pageId + "/road-details/999998")
            .param("millId", MILL).param("year", YEAR)
            .contentType(MediaType.APPLICATION_JSON).with(csrf()).content(edit))
  ...
  void editRestampsUpdateColumnsOnBothTables() throws Exception {
    ...
    // table's audit columns were asserted nowhere at all — and because they are nullable/defaulted in
    ...
    jdbc.update("UPDATE THE.ROAD_CONSTRUCTION_REPRT_DTL SET UPDATE_TIMESTAMP = DATE '2000-01-01',"
        + " UPDATE_USERID = 'STALE' WHERE ROAD_CONSTRUCTION_REPRT_DTL_ID = ?", detailId);
    jdbc.update("UPDATE THE.ILCR_COST_REPORT_DETAIL SET UPDATE_TIMESTAMP = TIMESTAMP"
  ...
  void unchangedClassificationPreservesMoistureCodes() throws Exception {
    ...
    // dropdown LISTS and never assigns, so the codes move only when the user picks new ones. BEC 8802
    ...
    jdbc.update("UPDATE THE.ROAD_CONSTRUCTION_REPRT_DTL"
  ...
  void omittedEngineeringIndicatorDefaultsToN() throws Exception {
    ...
    // pageDtlECIncludeCosts is a two-item dropdown with no empty option, so its effective default is
