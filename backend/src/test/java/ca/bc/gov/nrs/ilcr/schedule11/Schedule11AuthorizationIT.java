 ...
 * {@link CognitoGroupsJwtAuthenticationConverter} (the {@code jwt()} default reads only the
 * {@code scope} claim, so the converter is plugged into {@code .authorities(...)}) — the
 * established Boot-4 idiom from {@code Schedule1AuthorizationIT}.
 */
