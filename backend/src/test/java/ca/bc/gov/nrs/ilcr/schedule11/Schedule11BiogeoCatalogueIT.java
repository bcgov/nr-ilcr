 ...
 * slice S16). A read-only, global (no mill/year) type-ahead over {@code THE.BIOGEOCLIMATIC_CATALOGUE}:
 * a case-insensitive PREFIX match of {@code q} on the concatenated zone+subzone+variant+phase label
 * (the SAME concat the served location rows use), label-ordered and capped. A blank/whitespace
 * {@code q} returns {@code []}.
 *
 * <p>Security is ON for the whole class (as {@link Schedule11AuthorizationIT}) so the real
