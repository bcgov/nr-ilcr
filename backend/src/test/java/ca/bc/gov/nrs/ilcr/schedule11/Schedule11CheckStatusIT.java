 ...
 * when met. Per-missing-cost flags are composed VERBATIM in legacy order with the double space
 * after {@code location}. Fixtures are distinct from the write IT's (616 all-met, 617 missing, 613
 * empty) so the shared container's write mutations never perturb them.
 */
...
class Schedule11CheckStatusIT extends AbstractOracleIT {
  ...
  void allRequirementsMet() throws Exception {
    mockMvc
        .perform(post(CHECK_STATUS).with(csrf()).param("millId", "616").param("year", "2021"))
  ...
  @Test
  @DisplayName(
      "S05/S06: missing actual + missing planned -> FLD-004 flags, legacy order, no SUC-003")
  void missingCostsFlagged() throws Exception {
    mockMvc
        .perform(post(CHECK_STATUS).with(csrf()).param("millId", "617").param("year", "2021"))
        .andExpect(status().isOk())
        ...
        .andExpect(
            jsonPath(
                "$.errors[0].text", is("location  : Missing Actual - Actual cost: Value Required")))
        .andExpect(
            jsonPath(
                "$.errors[1].text",
  ...
  void zeroLocations_vacuouslyMet() throws Exception {
    ...
    mockMvc
        .perform(post(CHECK_STATUS).with(csrf()).param("millId", "613").param("year", "2021"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.requirementsMet", is(true)))
        .andExpect(jsonPath("$.errors", hasSize(0)))
        .andExpect(
  ...
  @Test
  @DisplayName(
      "AC9: check-status mutates nothing (row counts + every REVISION_COUNT identical, doc identical)")
  void mutatesNothing() throws Exception {
    ...
    String bsrRevisions =
        "SELECT BASIC_SILVICULTURE_REPORT_ID, REVISION_COUNT"
