...
/**
 * Story 25.1 acceptance — mill/year context guards for {@code GET /api/v1/schedule11} (AC3/AC4/AC5,
 * slices S11/S12/S13; AD-4, AD-8). All errors are RFC 7807 {@code application/problem+json} with
 * VERBATIM legacy text — including ERR-001's real trailing space ({@code messages.properties:18} in
 * the legacy bundle). Security is OFF (mock {@code ILCR_SUBMITTER}) so these isolate the guards,
 * not authz.
 *
 * <p>The 404 keys on the {@code ILCR_MILL_REPORT_STATUS} row (legacy {@code Schedule11MB.init()}
 * &rarr; {@code scheduleNotFound}) — NEVER on zero locations, which is a valid 200 ({@link
 * Schedule11DocumentIT} AC2). This is the list-schedule divergence from Schedule 1's summary-keyed
 * 404.
 */
...
class Schedule11ContextGuardIT extends AbstractOracleIT {
  ...
  void missingYear_returns400_verbatimMessage() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("millId", "610"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(PROBLEM_JSON))
        .andExpect(jsonPath("$.detail", is(ERR_001)));
  }
  ...
  void blankYear_returns400_verbatimMessage() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("millId", "610").param("year", " "))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(PROBLEM_JSON))
        .andExpect(jsonPath("$.detail", is(ERR_001)));
  }
  ...
  void seededMillUnseededYear_returns404() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("millId", "610").param("year", "1999"))
        .andExpect(status().isNotFound())
        .andExpect(content().contentTypeCompatibleWith(PROBLEM_JSON))
        .andExpect(jsonPath("$.detail", is(ERR_003)));
  }
