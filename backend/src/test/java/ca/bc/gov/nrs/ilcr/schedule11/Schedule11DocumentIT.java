 ...
 *
 * <p>Jackson serializes non-null only, so "absent = null" — nulls are asserted with {@code
 * doesNotExist()} (document {@code revisionCount} is ALWAYS null: no {@code ILCR_REPORT_SUMMARY}
 * row exists for a list schedule; per-row nulls follow BR-08).
 */
...
class Schedule11DocumentIT extends AbstractOracleIT {
  ...
  @Test
  @DisplayName(
      "610/2021 Draft — full pinned document, BR-08 derivations, sort, becLabel concat (AC1)")
  ...
  @Test
  @DisplayName(
      "612/2021 1-10 track Submitted, silviculture Draft — editable:true (AC6/S10 track independence)")
