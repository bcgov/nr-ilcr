 ...
 * POST/PUT/DELETE, {@code VIEW_SCHEDULE} on check-status), authorities via the production
 * {@link CognitoGroupsJwtAuthenticationConverter}.
 *
 * <p>Bodies on the 403 write tests are VALID because {@code @Valid} body binding runs during argument
 * resolution BEFORE {@code @PreAuthorize} fires — an invalid body would yield 400, not the 403 under
 * test. The "authorized" proof POSTs to a non-Draft mill (615/'S'): authz passes so the request is
 * NOT 403, and the service's Draft gate rejects it 409 WITHOUT mutating anything — no fixture churn.
 *
...
class Schedule11WriteAuthorizationIT extends AbstractOracleIT {
    ...
    void deleteLocation_noGroup_returns403() throws Exception {
        mockMvc.perform(delete(LOCATIONS + "/9201").with(csrf()).param("millId", "614").param("year", "2021")
                        .with(jwtWithGroups(List.of())))
                .andExpect(status().isForbidden());
    }
