 ...
 * in (the story's recorded merge-regression guard).
 *
...
class Schedule11WriteIT extends AbstractOracleIT {
  ...
  void addLocation_persistsImmediately_andIsDurable() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.locations[?(@.location=='North Ridge')].enhancedIndicator", contains(true)));
  ...
  void editLocation_persistsAndBumpsRevision() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.locations[?(@.locationId==9201)].revisionCount", contains(currentRevision + 1)));
  }
    ...
    mockMvc
        .perform(
            post(LOCATIONS)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"location\":\"X\",\"biogeoclimaticCatalogueId\":8801,\"netArea\":10.0}"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(PROBLEM_JSON))
    ...
    mockMvc
        .perform(
            post(LOCATIONS)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"location\":\"X\",\"enhancedIndicator\":true,\"biogeoclimaticCatalogueId\":8801}"))
        .andExpect(status().isBadRequest())
    ...
    mockMvc
        .perform(
            post(LOCATIONS)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"location\":\"X\",\"enhancedIndicator\":true,\"biogeoclimaticCatalogueId\":8801,\"netArea\":1000000.0}"))
        .andExpect(status().isBadRequest())
    ...
    mockMvc
        .perform(
            post(LOCATIONS)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"location\":\"X\",\"enhancedIndicator\":true,\"biogeoclimaticCatalogueId\":8801,\"netArea\":10.0,\"actualCost\":100000000}"))
        .andExpect(status().isBadRequest())
        .andExpect(
  ...
  @Test
  @DisplayName(
      "S18: NAR with more than one decimal -> 400 (never silently rounded by the column scale)")
    ...
    mockMvc
        .perform(
            post(LOCATIONS)
                ...
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"location\":\"X\",\"enhancedIndicator\":true,\"biogeoclimaticCatalogueId\":8801,\"netArea\":100.55}"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.detail", is("Entered NAR (ha) must be between 0 and 999,999.9.")));
  }
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.detail",
                is(
                    "Biogeo/Subzone/Variant code is invalid. "
  ...
  void duplicateBiogeoLocation_returns409() throws Exception {
    // 9201 is (2021, 614, '11', 8801, 'Existing Ridge') — re-adding that key trips
    // BSRPT_BSRPT_UK_UK.
    String body =
        """
