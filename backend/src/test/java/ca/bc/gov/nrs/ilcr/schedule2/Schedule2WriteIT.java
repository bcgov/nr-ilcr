 ...
 *
 * <p>Mutating tests use DEDICATED mills seeded by {@code V11} so they never clobber the read-only
 * Schedule 2 fixtures (621/517) the read *IT classes assert against (the container + data are
 * shared across the whole run): mill 622 / summary 1222 (update + stale + clear-null + delete),
 * mill 623 (Submitted — non-Draft write gate), mill 515 (create-on-save, no summary). Cases read
 * the revision at runtime so they stay order-independent.
 */
...
class Schedule2WriteIT extends AbstractOracleIT {
  ...
  @Test
  @DisplayName(
      "update — 622 Draft PUT persists 25/26, bumps revision, echoes recomputed doc + save msg")
  void put_update_persistsRecomputesBumps() throws Exception {
    int before = revisionOf(1222);
    mockMvc
        .perform(
            put(ENDPOINT)
                .param("millId", "622")
        ...
        .andExpect(jsonPath("$.millId", is(622)))
    ...
    assertEquals(before + 1, revisionOf(1222), "REVISION_COUNT must advance by exactly one");
  }
  ...
  @Test
  @DisplayName(
      "create — 515 Draft, no summary, revisionCount 0 -> new summary created, rev becomes 1")
  ...
  @Test
  @DisplayName("clear — 622 PUT with item 25 cost null clears the stored value to null")
  void put_clearItem25_persistsNull() throws Exception {
    ...
    assertEquals(
        ...
        JdbcTestUtils.countRowsInTableWhere(
            jdbcTemplate,
            DETAIL,
            "ILCR_REPORT_SUMMARY_ID = 1222 AND ILCR_REPORT_COST_ITEM_ID = 25 AND COST IS NULL"),
  ...
  void put_item25Over_returns400() throws Exception {
    expect400NothingPersisted(
        body(revisionOf(1222), 100000000, 1000, 5000),
  ...
  void put_item26VolumeOver_returns400() throws Exception {
    expect400NothingPersisted(
        body(revisionOf(1222), 5000, 10000000, 5000),
  ...
  void put_item26VolumeNegative_returns400() throws Exception {
    expect400NothingPersisted(
        body(revisionOf(1222), 5000, -1, 5000), "Entered volume must be between 0 and 9,999,999.");
  }
  ...
  void put_item26CostOver_returns400() throws Exception {
    expect400NothingPersisted(
        body(revisionOf(1222), 5000, 1000, 1000000000),
        "Entered cost must be between -999,999,999 and 999,999,999.");
  }

  @Test
  @DisplayName(
      "item 26 cost = 999,999,999 (widened costSize 9) PASSES; item 25 = 99,999,999 also OK")
  ...
  @Test
  @DisplayName(
      "S16: two fields out of range -> 400 returns BOTH verbatim messages, nothing persisted")
  void put_multipleFieldsOutOfRange_returnsAllMessages() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.detail", containsString("Entered volume must be between 0 and 9,999,999.")));
    assertEquals(before, revisionOf(1222), "a rejected multi-error PUT must not persist anything");
  }
  ...
  private void expect400NothingPersisted(String requestBody, String verbatimDetail)
      throws Exception {
    int before = revisionOf(1222);
    mockMvc
        .perform(
    ...
    assertEquals(before, revisionOf(1222), "a rejected PUT must not persist anything");
  }
  ...
  @Test
  @DisplayName("PUT against non-Draft (mill 623, track S) -> 409, no change")
  void put_nonDraft_returns409() throws Exception {
    int before = revisionOf(1223);
    mockMvc
        .perform(
            put(ENDPOINT)
                .param("millId", "623")
                .param("year", "2021")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body(before, 1, 1, 1)))
        .andExpect(status().isConflict())
        .andExpect(content().contentTypeCompatibleWith("application/problem+json"))
        .andExpect(
            jsonPath("$.detail", is("This schedule cannot be edited in its current status.")));
    assertEquals(before, revisionOf(1223), "a gated PUT must not change data");
  }

  @Test
  @DisplayName("DELETE against non-Draft (mill 623, track S) -> 409, row survives")
  void delete_nonDraft_returns409() throws Exception {
    mockMvc
        .perform(delete(ENDPOINT).param("millId", "623").param("year", "2021"))
        .andExpect(status().isConflict())
        .andExpect(
            jsonPath("$.detail", is("This schedule cannot be edited in its current status.")));
    assertEquals(
        1,
        JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, SUMMARY, "ILCR_REPORT_SUMMARY_ID = 1223"),
        "gated DELETE must not remove the row");
  }
  ...
  void put_staleRevision_returns409_thenRetrySucceeds() throws Exception {
    ...
    mockMvc
        .perform(
            put(ENDPOINT)
                ...
                .param("year", "2021")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body(n, 500000, 2000, 100000)))
        .andExpect(status().isOk())
    ...
    assertEquals(n + 1, revisionOf(1222), "the stale PUT must not overwrite");
    ...
    mockMvc
        .perform(
            put(ENDPOINT)
                ...
                .param("year", "2021")
                .contentType(MediaType.APPLICATION_JSON)
                .content(body(n + 1, 500000, 2000, 100000)))
        .andExpect(status().isOk())
  ...
  @Test
  @DisplayName(
      "DELETE — 625 Draft removes summary + 25/26; re-GET is the empty editable doc; idempotent")
  void delete_removesWholeSchedule() throws Exception {
    // Precondition: 625 has a summary + details.
    mockMvc
    ...
    assertEquals(
        0,
        JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, SUMMARY, "ILCR_REPORT_SUMMARY_ID = 1225"),
        "summary row must be gone");
    assertEquals(
        0,
        JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, DETAIL, "ILCR_REPORT_SUMMARY_ID = 1225"),
        "all detail rows must be gone");
