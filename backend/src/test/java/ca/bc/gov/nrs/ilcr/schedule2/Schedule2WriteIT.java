 ...
 * Schedule 2 fixtures (621/517) the read *IT classes assert against (the container + data are shared
 * across the whole run): mill 622 / summary 1222 (update + stale + clear-null + delete), mill 623
...
class Schedule2WriteIT extends AbstractOracleIT {
  ...
  @Test
  @DisplayName("update — 622 Draft PUT persists 25/26, bumps revision, echoes recomputed doc + save msg")
  void put_update_persistsRecomputesBumps() throws Exception {
    int before = revisionOf(1222);
    mockMvc.perform(put(ENDPOINT).param("millId", "622").param("year", "2021")
  ...
  void put_item25Over_returns400() throws Exception {
    expect400NothingPersisted(body(revisionOf(1222), 100000000, 1000, 5000),
  ...
  void put_item26VolumeOver_returns400() throws Exception {
    expect400NothingPersisted(body(revisionOf(1222), 5000, 10000000, 5000),
  ...
  void put_item26VolumeNegative_returns400() throws Exception {
    expect400NothingPersisted(body(revisionOf(1222), 5000, -1, 5000),
        "Entered volume must be between 0 and 9,999,999.");
  }
  ...
  void put_item26CostOver_returns400() throws Exception {
    expect400NothingPersisted(body(revisionOf(1222), 5000, 1000, 1000000000),
  ...
  void put_nonDraft_returns409() throws Exception {
    ...
    mockMvc.perform(put(ENDPOINT).param("millId", "623").param("year", "2021")
  ...
  void delete_nonDraft_returns409() throws Exception {
    mockMvc.perform(delete(ENDPOINT).param("millId", "623").param("year", "2021"))
        .andExpect(status().isConflict())
        .andExpect(jsonPath("$.detail", is("This schedule cannot be edited in its current status.")));
    assertEquals(1, JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, SUMMARY,
        "ILCR_REPORT_SUMMARY_ID = 1223"), "gated DELETE must not remove the row");
  }
  ...
  @Test
  @DisplayName("DELETE — 625 Draft removes summary + 25/26; re-GET is the empty editable doc; idempotent")
  void delete_removesWholeSchedule() throws Exception {
    // Precondition: 625 has a summary + details.
    mockMvc.perform(delete(ENDPOINT).param("millId", "625").param("year", "2021"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message.text", is("Data deleted successfully")));
    assertEquals(0, JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, SUMMARY,
        "ILCR_REPORT_SUMMARY_ID = 1225"), "summary row must be gone");
    assertEquals(0, JdbcTestUtils.countRowsInTableWhere(jdbcTemplate, DETAIL,
        "ILCR_REPORT_SUMMARY_ID = 1225"), "all detail rows must be gone");
    // Re-GET of a Schedule 2 with no summary is the empty editable document, NOT 404.
    mockMvc.perform(get(ENDPOINT).param("millId", "625").param("year", "2021")
            .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
