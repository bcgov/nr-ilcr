...
/**
 * Acceptance test — Story 4.4 Included Unacceptable Costs sub-resource (item-38 rows, AD-5).
 * Security OFF (mock ILCR_SUBMITTER). Read mill 574 (1 seeded row + Annual Rents S111), write mill
 * 575 (empty).
 */
@DisplayName(
    "/api/v1/schedule3/included-unacceptable-costs — Unacceptable sub-resource (Story 4.4)")
class Schedule3UnacceptableIT extends AbstractOracleIT {
  ...
  void addUpdateDelete_lifecycle() throws Exception {
    ...
    mockMvc
        .perform(delete(ENDPOINT + "/" + id).param("millId", "575").param("year", "2021"))
        .andExpect(status().isOk())
  ...
  void unknownId_returns404() throws Exception {
    mockMvc
        .perform(delete(ENDPOINT + "/999999").param("millId", "575").param("year", "2021"))
