 ...
 * Schedule4AuthorizationIT}). (Explicit here because the merged runtime default is fail-closed
 * {@code true}, which would require a {@code JwtDecoder} the non-auth IT does not supply.) Asserts
 * the pinned wire contract and the server-computed perUnit against the V7 seed.
 */
...
class Schedule4DocumentIT extends AbstractOracleIT {
  ...
  @Test
  @DisplayName(
      "514/2021 Draft — two locations, category grid with server-computed perUnit, ordered")
  void draftContext_listsLocationsWithCategories() throws Exception {
    mockMvc
        ...
        // Location "Harbour Dump" — a family of reports (7001 primary + 7011/7012) collapsed to
        // one.
        // id = the distance-null primary report (7001) — the rename-safe write handle (§Decision
        // 2).
        ...
        // 40 Lakeside Dry Dump (FIXED, primary report): vol 2000 / cost 100000 / perUnit 50.0, no
        // distance.
        ...
        // 47 Truck Barge/Ferry (DISTANCE): its OWN report's distance 120.5; vol 500 / cost 25000 /
        // 50.0.
        ...
        // 52 Rail Haul (DISTANCE): its OWN report's distance 88.5 — DIFFERENT from 47's 120.5,
        // proving
        ...
        // Harbour Dump's sub-page list (Story 4.3): the 43 Towing row (report 7013) — its OWN
        // report
        // sharing the location name, surfaced as a subPageRow (NOT a category), with its
        // description.
        ...
        // 50.0 naturalizes to the integer 50 (whole-value wire-contract parity, like other
        // distances).
