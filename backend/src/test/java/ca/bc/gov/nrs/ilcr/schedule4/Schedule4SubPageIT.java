...
/**
 * Acceptance test — Story 4.3. Sub-page list rows: read extension + POST (add) / DELETE (remove
 * row).
 *
 * <p>Security OFF ({@code ilcr.security.enabled=false}) so the mock {@code ILCR_SUBMITTER}
 * principal holds EDIT_SCHEDULE (403 case in {@link Schedule4WriteAuthorizationIT}); mutating
 * requests send {@code .with(csrf())} (the merged chain enforces {@code csrf.spa()}). Dedicated V9
 * mills: 550 "Rowed Dump" (primary 8050 + seeded Towing 8051 / Rehaul 8052 / Other 8053) for
 * read/add/delete, 551 (non-Draft) for the write gate. Cases read counts at runtime so they stay
 * order-independent.
 */
...
class Schedule4SubPageIT extends AbstractOracleIT {
  ...
  private static String rowBody(
      String type, String description, Object distance, Object volume, Object cost, Object cycle) {
    return """
        {
          "type": %s,
          "description": %s,
  ...
  void get_includesSubPageRows() throws Exception {
    mockMvc
        .perform(
            get("/api/v1/schedule4")
                .param("millId", "550")
                .param("year", "2021")
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        // one location; assert the seeded rows by their unique descriptions (order/count
        // independent).
        .andExpect(jsonPath("$.locations[0].name", is("Rowed Dump")))
        .andExpect(
            jsonPath(
                "$.locations[0].subPageRows[?(@.description=='Existing Towing')].code",
                contains(43)))
        .andExpect(
            jsonPath(
                "$.locations[0].subPageRows[?(@.description=='Existing Rehaul')].code",
                contains(46)))
        .andExpect(
            jsonPath(
                "$.locations[0].subPageRows[?(@.description=='Existing Rehaul')].cycle",
                contains(7)))
        .andExpect(
            jsonPath(
                "$.locations[0].subPageRows[?(@.description=='Existing Other')].code",
                contains(55)))
  ...
  void post_towing_persists() throws Exception {
    ...
    assertEquals(
        1,
        rows("ILCR_MILL_ID = 550 AND LOCATION_DESCRIPTION = 'Rowed Dump' " + "AND DISTANCE = 35.0"),
        "new row report carries its own distance");
    assertEquals(
        1,
        org.springframework.test.jdbc.JdbcTestUtils.countRowsInTableWhere(
            jdbcTemplate,
            DETAIL,
            "ILCR_REPORT_COST_ITEM_ID = 43 AND ITEM_DESCRIPTION = 'Added Towing'"),
  ...
  void post_truckRehaul_persistsCycle() throws Exception {
    ...
    assertEquals(
        1,
        rows(
            "ILCR_MILL_ID = 550 AND LOCATION_DESCRIPTION = 'Rowed Dump' "
                + "AND DISTANCE = 45.0 AND TRANSPORTATION_CYCLE_TIME = 12"),
        "cycle persisted on the report");
  }
  ...
  void post_other_persists() throws Exception {
    ...
    assertEquals(
        ...
        org.springframework.test.jdbc.JdbcTestUtils.countRowsInTableWhere(
            jdbcTemplate,
            DETAIL,
            "ILCR_REPORT_COST_ITEM_ID = 55 AND ITEM_DESCRIPTION = 'Added Other'"));
  }
  ...
  void post_volumeOver_returns400() throws Exception {
    expect400(
        rowBody("TOWING", "Bad Vol", "10.0", 1000000, 5000, null),
  ...
  void post_costOver_returns400() throws Exception {
    expect400(
        rowBody("TOWING", "Bad Cost", "10.0", 100, 10000000, null),
  ...
  void post_cycleOver_returns400() throws Exception {
    expect400(
        rowBody("TRUCK_REHAUL", "Bad Cycle", "10.0", 100, 5000, 1000000),
  ...
  @Test
  @DisplayName(
      "delete — removes the Towing row 8051 + detail; deleted echo; second delete idempotent")
  void delete_removesRow_idempotent() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/schedule4/locations/8050/rows/8051")
                .with(csrf())
                .param("millId", "550")
                .param("year", "2021"))
        .andExpect(status().isOk())
    ...
    assertEquals(
        0,
        org.springframework.test.jdbc.JdbcTestUtils.countRowsInTableWhere(
            jdbcTemplate, DETAIL, "TRANSPORTATION_REPORT_ID = 8051"),
        "its cascaded detail is gone");
  ...
  @Test
  @DisplayName(
      "delete guard — a primary/category report id via the row endpoint is a NO-OP (survives)")
  void delete_primaryReportId_isNoOp() throws Exception {
    // 8050 is Rowed Dump's PRIMARY (fixed-category) report, NOT a sub-page row.
    mockMvc
        .perform(
            delete("/api/v1/schedule4/locations/8050/rows/8050")
                .with(csrf())
                .param("millId", "550")
                .param("year", "2021"))
        .andExpect(status().isOk());
    assertEquals(
        1,
        rows("TRANSPORTATION_REPORT_ID = 8050"),
  ...
  void post_nonDraft_returns409() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/schedule4/locations/8060/rows")
                .with(csrf())
                .param("millId", "551")
                .param("year", "2021")
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void delete_nonDraft_returns409() throws Exception {
    mockMvc
        .perform(
            delete("/api/v1/schedule4/locations/8060/rows/8061")
                .with(csrf())
                .param("millId", "551")
                .param("year", "2021"))
        .andExpect(status().isConflict())
