...
class Schedule4WriteIT extends AbstractOracleIT {
  ...
  void put_create_insertsPrimaryAndDistanceChild() throws Exception {
            ...
            put(ENDPOINT)
                ...
                .content(
                    body(
                        ...
                        null,
                        "New Dump",
                        cat(40, 1000, 50000, null),
                        cat(47, 200, 8000, "60.0")))
                .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.message.text", is("Data saved successfully")))
        // The recomputed doc lists the created location (mill 541 accumulates create-test
        // locations,
        // so assert by name-filter, not a positional index — the DB counts below are the tight
        // check).
    ...
    assertEquals(
        2,
        reports(
            "ILCR_MILL_ID = 541 AND ILCR_CATEGORY_ID = '4' "
                + "AND LOCATION_DESCRIPTION = 'New Dump'"),
        "primary + distance child persist");
    assertEquals(
        1,
        reports(
            "ILCR_MILL_ID = 541 AND LOCATION_DESCRIPTION = 'New Dump' " + "AND DISTANCE = 60.0"),
        "the distance child carries its own distance");
    assertEquals(
        1,
        reports(
            "ILCR_MILL_ID = 541 AND LOCATION_DESCRIPTION = 'New Dump' " + "AND DISTANCE IS NULL"),
        "the fixed primary has no distance");
  }
  ...
  void put_create_nameOnly() throws Exception {
    ...
    assertEquals(
        1,
        reports("ILCR_MILL_ID = 541 AND LOCATION_DESCRIPTION = 'Bare Landing'"),
        "name-only location = one primary report");
    assertEquals(
        0,
        details(
            "TRANSPORTATION_REPORT_ID IN "
                + "(SELECT TRANSPORTATION_REPORT_ID FROM "
                + REPORT
  ...
  void put_edit_updatesInPlace() throws Exception {
    ...
    assertEquals(
        1,
        details(
            "TRANSPORTATION_REPORT_ID = 8001 AND ILCR_REPORT_COST_ITEM_ID = 40 "
                + "AND VOLUME = 1500 AND COST = 60000"),
        "fixed detail updated in place");
    assertEquals(
        1,
        reports("TRANSPORTATION_REPORT_ID = 8002 AND DISTANCE = 70.0"),
  ...
  void put_rename_reStampsFamily() throws Exception {
    ...
    assertEquals(
        1,
        reports("ILCR_MILL_ID = 545 AND LOCATION_DESCRIPTION = 'Renamed Landing'"),
        "family re-stamped to the new name");
    assertEquals(
        0,
        reports("ILCR_MILL_ID = 545 AND LOCATION_DESCRIPTION = 'Renamable Dump'"),
  ...
  void put_blankName_returns400() throws Exception {
    mockMvc
        ...
        .andExpect(
            jsonPath(
                "$.detail", is("Location Name can not be empty. Please enter a description.")));
  }
  ...
  void put_duplicateName_returns409() throws Exception {
    ...
    assertEquals(
        0,
        reports("ILCR_MILL_ID = 546 AND LOCATION_DESCRIPTION = 'rival dump'"),
  ...
  void put_volumeOver_returns400() throws Exception {
    expect400(
        body(8001, revisionOf(8001), "Existing Dump", cat(40, 10000000, 5000, null)),
  ...
  void put_costOver_returns400() throws Exception {
    expect400(
        body(8001, revisionOf(8001), "Existing Dump", cat(40, 5000, 100000000, null)),
  ...
  void put_distanceWithoutAmounts_returns400() throws Exception {
    expect400Contains(
        body(8001, revisionOf(8001), "Existing Dump", cat(47, null, null, "50.0")),
        "Value Required");
  }
  ...
  void delete_removesFamily_idempotent() throws Exception {
    ...
    assertEquals(
        0,
        reports("ILCR_MILL_ID = 544 AND LOCATION_DESCRIPTION = 'Deletable Dump'"),
        "primary + distance child both removed");
    assertEquals(
        0, details("TRANSPORTATION_REPORT_ID IN (8030, 8031)"), "all cascaded detail rows removed");
