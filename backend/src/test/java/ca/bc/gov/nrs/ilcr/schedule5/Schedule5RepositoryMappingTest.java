 ...
 * into the service-facing {@link DetailRow}. No database: the query itself is pinned by {@code
 * Schedule5RepositoryIT}, which surefire cannot run (CI runs no Oracle ITs — AR17), so the
    ...
    when(repository.findSubPageRowEntities(CAMP, ITEM_CAMP_ROW, 690L, 2016))
        .thenReturn(
            List.of(
                ...
                new CostReportDetailEntity(
                    8725, CAMP, ITEM_CAMP_ROW, new BigDecimal("120000"), 300, "Propane")));
