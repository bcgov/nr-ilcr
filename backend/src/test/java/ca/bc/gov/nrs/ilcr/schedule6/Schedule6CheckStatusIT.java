 ...
 * with 8325), 8325 complete, 8326 missing cost, 8327 missing supply block, 8328 missing both,
 * 8329 cost 0 (MET — the D2 quirk). Mill 663 is all-met; 515 zero-records; 660 lone-comment.
 * The endpoint mutates nothing — every test here is read-only by contract, proven by the
 * row-count + every-REVISION_COUNT fingerprint (GET-body equality could miss a column the
 * document doesn't expose — the 25.2 rationale).
 */
...
class Schedule6CheckStatusIT extends AbstractOracleIT {
    ...
    @Test
    @DisplayName("S09/S11/S20/S21: mixed results — byte-for-byte lines, per-record met banner, "
    ...
    void mixedResults_composedVerbatim_mutatesNothing() throws Exception {
        ...
        mockMvc.perform(post(CHECK_STATUS).with(csrf()).param("millId", "664").param("year", "2021"))
                ...
                .andExpect(jsonPath("$.records[?(@.recordId==8325)].metMessage.key",
                        contains("roadRequirementsMetMsg")))
                .andExpect(jsonPath("$.records[?(@.recordId==8325)].metMessage.text",
                ...
                .andExpect(jsonPath("$.records[?(@.recordId==8326)].issues[0].message.key",
                        contains("missingRequiredFieldMsg")))
                .andExpect(jsonPath("$.records[?(@.recordId==8326)].issues[0].message.text",
                        contains("Road : 2 - TSA or TFL (Cost $) : Value Required")))
                // S11: missing supply block.
                .andExpect(jsonPath("$.records[?(@.recordId==8327)].issues[0].message.text",
                        contains("Road : 3 - Supply Block : Value Required")))
                // S21: one record, BOTH lines, legacy order (supply block before cost).
                .andExpect(jsonPath("$.records[?(@.recordId==8328)].issues[0].message.text",
                        contains("Road : 4 - Supply Block : Value Required")))
                .andExpect(jsonPath("$.records[?(@.recordId==8328)].issues[1].message.text",
                ...
                .andExpect(jsonPath("$.records[?(@.recordId==8329)].metMessage.text",
    ...
    @Test
    @DisplayName("SUC-003: all records complete -> MET, the single schedule banner, NO per-record results")
    void allMet_singleBannerOnly() throws Exception {
        mockMvc.perform(post(CHECK_STATUS).with(csrf()).param("millId", "663").param("year", "2021"))
                .andExpect(status().isOk())
    ...
    void zeroRecords_vacuousPass() throws Exception {
        mockMvc.perform(post(CHECK_STATUS).with(csrf()).param("millId", "515").param("year", "2021"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.outcome", is("MET")))
                .andExpect(jsonPath("$.records", hasSize(0)));
    }
    ...
    @Test
    @DisplayName("Lone-comment placeholder -> vacuous MET (deviation (d): the hidden row is not flagged)")
    void loneComment_vacuousPass() throws Exception {
        ...
        mockMvc.perform(post(CHECK_STATUS).with(csrf()).param("millId", "660").param("year", "2021"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.outcome", is("MET")))
                .andExpect(jsonPath("$.messages[0].key", is("scheduleRequirementsMetMsg")))
                .andExpect(jsonPath("$.records", hasSize(0)));
    ...
    @Test
    @DisplayName("VIEW-gated, not Draft-gated: check-status runs on a non-Draft ('S') mill (2.6 precedent)")
    void nonDraftTrack_stillChecks() throws Exception {
        // Mill 662/2021 is 'S' with one complete record — the endpoint must NOT 409.
        mockMvc.perform(post(CHECK_STATUS).with(csrf()).param("millId", "662").param("year", "2021"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.outcome", is("MET")));
    }
    ...
    void missingContext_returns400Err001() throws Exception {
        mockMvc.perform(post(CHECK_STATUS).with(csrf()).param("year", "2021"))
     ...
     * 2026-08-04). The detail's own {@code REVISION_COUNT} is included for the same reason:
     * {@code updateCostDetail} deliberately never bumps it, so it too was invisible.
     */
