 ...
 * in {@link Schedule6AuthorizationIT}).
 *
 * <p>KEY (Story 8.1 Task 1): there is NO "no records" 404 — a valid, active mill/year with no
 * {@code ROAD_MAINTENANCE_REPORT} rows returns 200 (legacy-faithful, mirrors Schedule 4). The 404
 * is reserved for the missing mill/year context.
 */
...
class Schedule6ContextGuardIT extends AbstractOracleIT {
  ...
  void nonNumericYear_returns400_verbatim() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("millId", "514").param("year", "abc"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(PROBLEM_JSON))
        .andExpect(jsonPath("$.detail", is(ERR_001)));
  }
  ...
  void unknownMill_returns404_verbatim() throws Exception {
    mockMvc
        .perform(get(ENDPOINT).param("millId", "999999").param("year", SEEDED_YEAR))
