 ...
 * three writes, {@code VIEW_SCHEDULE} on check-status), authorities via the production
...
class Schedule6WriteAuthorizationIT extends AbstractOracleIT {
    ...
    void addRecord_noGroup_returns403() throws Exception {
        mockMvc.perform(post(RECORDS).with(csrf()).param("millId", "666").param("year", "2021")
    ...
    void updateRecord_noGroup_returns403() throws Exception {
        mockMvc.perform(put(RECORDS + "/8358").with(csrf()).param("millId", "666").param("year", "2021")
                        .contentType(MediaType.APPLICATION_JSON).content(VALID_BODY)
                        .with(jwtWithGroups(List.of())))
                .andExpect(status().isForbidden());
    }
    ...
    void saveGeneralComments_noGroup_returns403() throws Exception {
        mockMvc.perform(put(COMMENTS).with(csrf()).param("millId", "666").param("year", "2021")
