...
/**
 * Asserts which ACTION each Schedule 7B endpoint authorizes on, by reading its
 * {@code @PreAuthorize} annotation.
 *
 ...
 * {@code EDIT_SCHEDULE}, and every negative case in that IT uses a caller with no ILCR group at
 * all. So swapping a write's action to {@code VIEW_SCHEDULE} — or to a misspelled action name,
 * which {@code SchedulePermissions.hasPermission} silently DENIES rather than failing loudly —
 * passes every one of those tests. Until the role×status matrix lands (AR14 / Story 16.1) and the
 * two roles diverge, the annotation values are the only place the read/write distinction is
 * actually recorded, so they are asserted directly. Runs under surefire (no container), so it also
 * counts toward measured coverage.
 */
...
class Schedule7bAuthorizationContractTest {
  ...
  @Test
  @DisplayName(
      "Reads authorize VIEW_SCHEDULE, writes authorize EDIT_SCHEDULE — no endpoint unguarded")
  void everyEndpointNamesTheRightAction() {
    for (Map.Entry<String, String> expected : EXPECTED.entrySet()) {
      ...
      assertThat(annotation).as("%s carries @PreAuthorize", expected.getKey()).isNotNull();
  ...
  void noEndpointEscapesTheContract() {
    ...
    var endpoints =
        java.util.Arrays.stream(Schedule7bController.class.getDeclaredMethods())
            .filter(
                m ->
