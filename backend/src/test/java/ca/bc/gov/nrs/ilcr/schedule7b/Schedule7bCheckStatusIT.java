...
/**
 * Story 13.2 acceptance — {@code POST /api/v1/schedule7b/check-status} and the type-conditional matrix
 * (AC9, BR-07; slices S15-S20/S24/S26-S28). Runs against the V20260811 seed, which was built so each culvert
 * exercises a different branch:
 *
 * <ul>
 *   <li>7801 Round, complete — passes.
 *   <li>7802 Others, comments present, NO span — passes, because span is conditional on type {@code R}.
 *   <li>7803 Round, span/length/install missing and rise blank — flags exactly three lines, and NEVER a
 *       rise line.
 ...
 * </ul>
 *
 * <p><strong>Check Status is read-only and deliberately NOT Draft-gated</strong>, which is why the 517
 * (Submitted) case is expected to run rather than 409. Recorded deviation: legacy DISABLED both Check
 * Status buttons whenever the report was not editable ({@code schedule7B.xhtml:264-265,558-559},
 * {@code disabled="#{schedule7bMB.disableReportEdits()}"}). Gating the endpoint the same way would
 * break the report-level check — UC-CHK-001 requires calling each schedule's own validation method
 * during a status transition, i.e. precisely when the report is Submitted or Verified, and the epic
 * notes legacy's transition gate always evaluated 7B correctly. So the button-disable is reproduced in
 * the frontend from the document's {@code editable} flag (Story 13.3), and the endpoint stays open to
 * any {@code VIEW_SCHEDULE} holder. Security OFF.
 */
...
class Schedule7bCheckStatusIT extends AbstractOracleIT {
  ...
  void flagsOnlyTheIncompleteCulvert() throws Exception {
    mockMvc.perform(post(ENDPOINT).param("millId", "514").param("year", "2021")
        ...
        .andExpect(jsonPath("$.errors[*].text", contains(
  ...
  void riseIsNeverFlagged() throws Exception {
    mockMvc.perform(post(ENDPOINT).param("millId", "514").param("year", "2021")
        ...
        .andExpect(jsonPath("$.errors[*].text",
            everyItem(not(Matchers.containsString("Rise")))))
        .andExpect(jsonPath("$.errors[*].text",
            everyItem(not(Matchers.containsString("rise")))));
  }
  ...
  void nonRoundWithoutSpanPasses() throws Exception {
    // 7802 (rowCounter 2) is type 'O' with no span at all. Asserting the FULL error list rather than
    ...
    mockMvc.perform(post(ENDPOINT).param("millId", "514").param("year", "2021")
        ...
        .andExpect(jsonPath("$.errors[*].text", contains(
            ...
            "Culvert Report Id: 3 - Install Cost : Value Required")))
        .andExpect(jsonPath("$.errors[*].text",
            everyItem(not(Matchers.containsString(" 2 -")))));
  }
  ...
  void nonOthersWithoutCommentsPasses() throws Exception {
    // Retargeted from 7801, which HAS comments — so the old assertion held whether or not the comments
    ...
    mockMvc.perform(post(ENDPOINT).param("millId", "514").param("year", "2021")
        ...
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.errors[*].text",
            everyItem(not(Matchers.containsString("Comments")))))
  ...
  void multipleGapsComposeForOneCulvert() throws Exception {
    // 7803 alone contributes three lines (span, length, install cost) — the composition case. Tied to
    // rowCounter 3 explicitly: a bare count of 3 would also pass if the three lines belonged to three
    ...
    mockMvc.perform(post(ENDPOINT).param("millId", "514").param("year", "2021")
        ...
        .andExpect(jsonPath("$.errors.length()", is(3)))
        .andExpect(jsonPath("$.errors[*].text",
            everyItem(Matchers.containsString(" 3 -"))));
  }
}
