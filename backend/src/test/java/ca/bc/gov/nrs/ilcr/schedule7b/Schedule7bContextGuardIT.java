...
/**
 * Story 13.1 acceptance — the mill/year context guards on {@code GET /api/v1/schedule7b} (AC6, slices
 * S11/S12/S13). Every message is asserted VERBATIM including trailing whitespace (AD-8): the legacy
 * bundle's {@code millYearNotSelectedErrorMsg} genuinely ends in a space, and "fixing" it here would
 * mask a bundle regression. Security is OFF (mock {@code ILCR_SUBMITTER}).
 */
...
class Schedule7bContextGuardIT extends AbstractOracleIT {
  ...
  void emptyList_returns200() throws Exception {
    mockMvc.perform(get(ENDPOINT).param("millId", "515").param("year", "2021")
        ...
        // The Type dropdown must still be served for an empty schedule, or the Add form has no options.
