...
/**
 * Story 13.2 acceptance — culvert write paths for {@code /api/v1/schedule7b/culverts}
 * (AC1-AC8/AC10, slices S01-S10/S14/S21-S23/S25/S29). All mutations target mill 515 (active, Draft,
 * and initially empty of culverts) so they never disturb the seeded read/check-status fixtures on
 * 514/517. Security OFF; authz is proven in {@link Schedule7bAuthorizationIT}.
 */
...
class Schedule7bWriteIT extends AbstractOracleIT {
   ...
   * Schedule7bContextGuardIT.emptyList_returns200}, which asserts it empty — that guard must not
   ...
   * class's own {@code @AfterEach}, which two OTHER classes then depended on ({@code
   * Schedule7bContextGuardIT.emptyList_returns200}, {@code
   * Schedule7bCheckStatusIT.emptyScheduleIsAllMet}). One test dying before its cleanup ran, or
  ...
  /**
   * Create one culvert and return its generated id. Reads the LAST entry of the echoed document,
   * not the first: the document is ordered by {@code CULVERT_REPORT_ID} ascending and the ids come
   * from a sequence, so the culvert this call just created is always the last one — reading index 0
   * would hand back the earliest culvert and make two successive calls return the same id.
   */
  ...
  @Test
  @DisplayName(
      "AC1: BOTH cost rows are written even when both costs are blank (legacy storage shape)")
  void blankCostsStillWriteBothRows() throws Exception {
    ...
    // the legacy screen, which has no insert path on its update branch. Counted for THIS culvert's
    // id
    // rather than mill-wide, so a stray row from anywhere else cannot make this pass or fail
    // wrongly.
    ...
    var stored =
        jdbc.queryForMap(
            "SELECT ILCR_CULVERT_TYPE_CODE, CULVERT_PIECE_COUNT, COMMENTS, REVISION_COUNT "
                + "FROM THE.CULVERT_REPORT WHERE CULVERT_REPORT_ID = ?",
            id);
  ...
  void duplicateBatchEntryReturns400() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(status().isBadRequest())
        .andExpect(
            jsonPath("$.detail", is("The same culvert report was submitted more than once.")));
  }
  ...
  @Test
  @DisplayName(
      "AC4: deleting the LAST culvert still echoes SUC-002 — legacy 7B has no empty branch")
  void deleteLastCulvertStillReportsDeleted() throws Exception {
    ...
    mockMvc
        ...
        // anyDataToSaveInfoMsg ("Any data was saved. The Schedule is empty.") appears exactly once
        // in
        // the legacy source — Schedule7aMB.java:374. Legacy 7B's update() always emits the key it
        // was
  ...
  void deleteUnknownIdReturns404() throws Exception {
    mockMvc
        .perform(delete(CULVERTS + "/999999").param("millId", MILL).param("year", YEAR))
        .andExpect(status().isNotFound());
  }
  ...
  void nonDraftWritesReturn409() throws Exception {
    ...
    mockMvc
        .perform(delete(CULVERTS + "/7851").param("millId", "517").param("year", YEAR))
        .andExpect(status().isConflict());

    // Nothing was removed from the read fixture.
    Integer stillThere =
        jdbc.queryForObject(
            "SELECT COUNT(*) FROM THE.CULVERT_REPORT WHERE CULVERT_REPORT_ID = 7851",
            Integer.class);
  ...
  void spanOutOfRangeReturns400() throws Exception {
    postCulvert(
        ...
        .andExpect(status().isBadRequest())
        .andExpect(
            jsonPath("$.detail", containsString("Entered span must be between 0 and 9,999,999.")));
  }
  ...
  void nonNumericDimensionReturns400() throws Exception {
    postCulvert(
        ...
        // because GlobalExceptionHandler picked the converter message from the target Java TYPE, so
        // a
        // mistyped SPAN told the reporter their COST was invalid. A status-only assertion is
        // exactly
        // where that hid.
  ...
  void oversizedCommentsReturn400() throws Exception {
    postCulvert(
        ...
        .andExpect(status().isBadRequest())
        .andExpect(
            jsonPath("$.detail", containsString("Comments must be 3500 characters or fewer.")));
  }

  @Test
  @DisplayName(
      "AC8: a culvert type retired before the reporting year -> 400, and nothing is stored")
  ...
  void saveAllIsDraftGated() throws Exception {
    ...
    // saveAllCulverts left the whole suite green while a Submitted report could be mutated
    // wholesale
  ...
  void unchangedRetiredTypeDoesNotBlockTheSave() throws Exception {
    ...
    // another field must succeed — otherwise a single legacy row with a since-retired code would
    // 400
    ...
    jdbc.update(
        "UPDATE THE.CULVERT_REPORT SET ILCR_CULVERT_TYPE_CODE = 'XOLD', SPAN_SIZE = 700, "
  ...
  void deleteOfForeignMillCulvertReturns404() throws Exception {
    // The IDOR mirror of foreignMillCulvertReturns404, which only covered the PUT. Every other
    // delete
    ...
    mockMvc
        .perform(delete(CULVERTS + "/7801").param("millId", MILL).param("year", YEAR))
        .andExpect(status().isNotFound());
  ...
  void writeAgainstAnotherYearReturns404() throws Exception {
    // 7861 belongs to mill 680 / 2020. Addressing it under year=2021 must not resolve, which is
    // what
    ...
    mockMvc
        ...
        .andExpect(status().isNotFound());
    mockMvc
        .perform(delete(CULVERTS + "/7861").param("millId", "680").param("year", "2021"))
        .andExpect(status().isNotFound());
  ...
  void batchEntryWithoutRevisionCountReturns400() throws Exception {
    ...
    // Proves the OnUpdate group propagates through @Valid List<Item> into each CulvertRequest. If
    // it
    ...
    var afterInsert =
        jdbc.queryForMap(
            "SELECT ENTRY_USERID, UPDATE_USERID, ENTRY_TIMESTAMP, UPDATE_TIMESTAMP "
                + "FROM THE.CULVERT_REPORT WHERE CULVERT_REPORT_ID = ?",
            id);
    // Security is OFF in this class, so the principal is the configured mock user; assert the
    // columns
  ...
  void persistenceFailureRollsBackBothTables() throws Exception {
    ...
    // BYTE), so temporarily narrowing it makes the INSERT fail after the id has been drawn. What
    // this
    ...
    // A CHECK constraint on a sentinel value, not a column narrowing: the seeded fixtures already
    // hold
    ...
    jdbc.execute(
        "ALTER TABLE THE.CULVERT_REPORT ADD CONSTRAINT TMP_7B_FORCE_FAIL "
  ...
  @Test
  @DisplayName(
      "A comment inside the character cap but over the 4,000-byte column is a 400, not a 500")
