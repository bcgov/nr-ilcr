...
/**
 * Unit test for the Schedule 8 page write control flow (Story 14.2) — mocked repository, no DB.
 * Covers the Draft gate (409), the optimistic-lock stale path (409), create → insert + revision
 * bump, the TFL⇄supply-block normalization, and the idempotent delete guard. The full
 * create/edit/delete + cascade behaviour is proven end-to-end against Oracle in {@link
 * Schedule8PageWriteIT}.
 */
...
class Schedule8PageWriteServiceTest {
  ...
  void stubReadsForRecompute() {
    // savePage/deletePage recompute the document at the end via getSchedule8 — default its reads
    // empty.
    ...
    org.mockito.Mockito.lenient()
        .when(repository.regionLabels())
        .thenReturn(Map.of("R1", "Region"));
    org.mockito.Mockito.lenient()
        .when(repository.becZoneLabels())
        .thenReturn(Map.of("BZ1", "Zone"));
    org.mockito.Mockito.lenient()
        .when(repository.tsaNumberLabels())
        .thenReturn(Map.of("TSA5", "Tsa"));
    org.mockito.Mockito.lenient()
        .when(repository.supplyBlockLabels())
        .thenReturn(Map.of("B", "Block"));
    org.mockito.Mockito.lenient()
        .when(repository.tflNumberLabels())
        .thenReturn(Map.of("48", "Tfl"));
