 ...
 * (mock ILCR_SUBMITTER holds EDIT_SCHEDULE); mutating requests carry {@code .with(csrf())}.
 * Dedicated V13 mills/pages: 590/8900 create + validation, 591/8910/8911 edit/stale,
 * 592/8920/8921(+rate 8922) delete cascade, 593/8930 (non-Draft) gate. Explicitly proves the
 * S15-vs-S16 asymmetry: sum &gt; 100 is rejected at Save, sum &lt; 100 SAVES.
 */
...
class Schedule8SampleWriteIT extends AbstractOracleIT {
  ...
  private org.springframework.test.web.servlet.ResultActions putSample(
      int millId, int pageId, String body) throws Exception {
  ...
  void create_persistsSampleAtRevisionOne() throws Exception {
    ...
    assertEquals(
        ...
        jdbcTemplate.queryForObject(
            "SELECT REVISION_COUNT FROM "
                + SAMPLE
                + " WHERE TREE_TO_TRUCK_REPORT_ID = 8900 AND CONTRACTOR_ID = 'CNEW1'",
            Integer.class));
  }
  ...
  void delete_cascadesRateDetails() throws Exception {
    mockMvc
        .perform(
            delete(url(8920) + "/8921").param("millId", "592").param("year", "2021").with(csrf()))
        .andExpect(status().isOk())
    ...
    assertEquals(
        ...
        JdbcTestUtils.countRowsInTableWhere(
            jdbcTemplate, RATE, "TREE_TO_TRUCK_DETAIL_REPORT_ID = 8921"));
  }
  ...
  void deleteUnknown_isIdempotent() throws Exception {
    mockMvc
        .perform(
            delete(url(8920) + "/99999").param("millId", "592").param("year", "2021").with(csrf()))
        .andExpect(status().isOk());
  }
}
