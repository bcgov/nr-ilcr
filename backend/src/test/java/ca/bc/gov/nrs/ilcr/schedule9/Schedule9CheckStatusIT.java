 ...
 * the title, the preserved Save-vs-Check asymmetry (blank units/cost and a side slope of exactly 100
 * SAVE but are flagged here), and the base validator's {@code invalidRangeErrorMsg} for a range
  ...
  void issuesComposedVerbatim() throws Exception {
    mockMvc.perform(post(CHECK_STATUS).with(csrf()).param("millId", "703").param("year", "2021"))
        ...
        .andExpect(jsonPath("$.errors[*].text", contains(
            ...
            // record 9143 (row 2): side slope 100 SAVES (<=100) but Check flags it (>99), and the
            // range message is the base validator's invalidRangeErrorMsg, not the save-time FLD-003.
