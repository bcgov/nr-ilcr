 ...
 * SPACE. Security is pinned OFF explicitly (Story 8.1's lesson) so these isolate the context guards;
 * 403 is proven in {@link Schedule9AuthorizationIT}.
 *
 * <p>Like Schedules 4/5/6, Schedule 9 is summary-less: a valid ACTIVE mill/year with zero
 * {@code CONTRACTUAL_WORK_REPORT} rows returns 200 with {@code records: []} (mill 515 in
 * {@link Schedule9DocumentIT}), NOT a 404. The 404 is reserved for a missing mill/year context row.
 * These guards are delegated to the shared {@code MillContextService.validateMillYearActive}, so the
...
class Schedule9ContextGuardIT extends AbstractOracleIT {
  ...
  void nonNumericYear_returns400_verbatim() throws Exception {
    mockMvc.perform(get(ENDPOINT).param("millId", SEEDED_MILL).param("year", "xyz"))
        .andExpect(status().isBadRequest())
        .andExpect(content().contentTypeCompatibleWith(PROBLEM_JSON))
        .andExpect(jsonPath("$.detail", is(ERR_003)));
  }
