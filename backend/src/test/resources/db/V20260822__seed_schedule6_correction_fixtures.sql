...
-- Task 6 (Check Status over submitted values) fixtures. Mill 671 already exists as an ACTIVE mill
-- (V20260807 schedule 5 write fixtures — MILL + XREF inserted there for category '5'/year 2021), so
-- it is reused here for a DIFFERENT year and category rather than re-inserting MILL/XREF (which
-- would collide on the mill_id primary key). ROAD_MAINTENANCE_REPORT_ID 8399, ILCR_COST_REPORT_
-- DETAIL_ID 8400 (free: this file's own blocks end at 8398/8382 and V31/V32 only reach 8362).
-- Read-only endpoint (checkStatus mutates nothing), so both IT tests below safely share this one
-- context — no per-test isolation is needed the way a mutating write path would require.
INSERT INTO THE.ILCR_MILL_REPORT_STATUS (REPORT_YEAR, ILCR_MILL_ID, ILCR_MILL_REPORT_STATUS_CODE, ENTRY_USERID) VALUES (2020, 671, 'D', 'SEED');
INSERT INTO THE.ROAD_MAINTENANCE_REPORT (ROAD_MAINTENANCE_REPORT_ID, REPORT_YEAR, ILCR_MILL_ID, ILCR_CATEGORY_ID, TSA_NUMBER, TSB_NUMBER_CODE, TFL_NUMBER_CODE, COMMENTS, REVISION_COUNT, ENTRY_USERID, ENTRY_TIMESTAMP, UPDATE_USERID, UPDATE_TIMESTAMP)
  VALUES (8399, 2020, 671, '6', 'Y9', 'Y9A', NULL, 'stored comment, never touched by check-status', 0, 'SEED', SYSDATE, 'SEED', SYSDATE);
INSERT INTO THE.ILCR_COST_REPORT_DETAIL (ILCR_COST_REPORT_DETAIL_ID, ROAD_MAINTENANCE_REPORT_ID, ILCR_REPORT_COST_ITEM_ID, VOLUME, COST, COMMENTS, ENTRY_USERID)
