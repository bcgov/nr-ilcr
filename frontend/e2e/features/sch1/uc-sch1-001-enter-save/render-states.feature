...
Feature: Report Average Cost of Logging (Schedule 1) — context-guard render states
  ...
  @S21 @p1
  Scenario: A mill and year with no Schedule 1 summary suppresses the input form
    Given the Schedule 1 render-state anchor "no-schedule" is selected
    And I have selected that mill and reporting year on the Home page
    When I navigate to Schedule 1 expecting a guard
    Then I should see the error "No Schedule 1 exists for Mill 16050 in Reporting Year 2016. Select another mill/year from Home, or create Schedule 1 data for this context."
    And the Schedule 1 input form is not displayed
