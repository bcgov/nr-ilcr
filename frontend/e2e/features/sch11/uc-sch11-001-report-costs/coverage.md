...
- **The zero-write proofs had no settle window** — an app that rendered the error and *then* POSTed a tick
  later would have read 0 and passed. The tally is now read only after a deterministic barrier: the page's
  own deferrals are drained (microtask → frame → macrotask), then a real round-trip completes through the
  page's network stack, and route spies fire at request initiation — so anything the rejected action
  started is already counted. No wall-clock constant, so it neither flakes on a slow runner nor pads the
  suite (`settleBeforeReadingSpy`, steps/sch11/schedule11.steps.ts).
...
| S08 Cancel delete confirmation — row unchanged | S08.feature (Alt, gap-analysis 3d) | `Modal onRequestClose` (index.tsx:1045) | `delete.feature` `@S08 @p1` | covered | Proves the negative four ways: zero mutations (spy, read after the deferral+round-trip barrier), unchanged row count, `revisionCount` still 0, and the delete-success message absent |
...
| Duplicate biogeo/location key → 409 | **no legacy slice** (SPEC-1) | `Schedule11Service.addLocation` duplicate check | — | not-applicable (UI) | **GAP-4 CLOSED** — unreachable through the UI (forced selection); covered by `Schedule11WriteIT.duplicateBiogeoLocation_returns409()`, which runs in CI (`analysis.yml` passes `-Dskip.integration.tests=false`) |
| Unresolvable BEC id → 400 `invalidBiogeoCode` | **no legacy slice** (SPEC-1) | `Schedule11Service` biogeo resolution | — | not-applicable (UI) | **GAP-4 CLOSED** — same reason; covered by `Schedule11WriteIT.unresolvableBiogeo_returns400()`, likewise CI-gated |
