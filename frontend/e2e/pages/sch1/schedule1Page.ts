...
export class Schedule1Page {
  ...
  fieldIdFor(label: string): string {
    ...
    if (match) {
      ...
      if (ids) {
        ...
        if (!ids.cost) {
          throw new Error(
            `Schedule 1 line item "${match[1]}" has no editable cost (pulled/derived).`,
          )
        }
        return ids.cost
      }
  ...
  /**
   * EVERY editable amount input on the statement — the `vol-*` / `cost-*` ids the form seeds from.
   * Used by the #296 "every amount is blank" assertion, which needs all of them rather than the one
   * representative box `firstAmountInput` gives (#296 code review).
   */
  get allAmountInputs(): Locator {
    return this.page.locator('input[id^="vol-"], input[id^="cost-"]')
  }
