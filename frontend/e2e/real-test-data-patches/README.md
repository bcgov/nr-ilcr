...
| `sch4/unsaved-check-anchors.sql` (+ `.teardown.sql`) | `UC-SCH4-001` | One dedicated Draft mill-year for the BR-12 / #359 scenario. Same reason, and Schedule 4's preflight is the strictest — it enforces one anchor per mutating scenario *and* "used in at most one feature file". **No longer referenced since 2026-09-18 (#465):** Schedule 4's Check Status can no longer flag any saved state, so its unsaved-panel scenario was retired (sch4 `defects.md` DIV-8). The patch is kept so the teardown can be applied to a seeded environment; nothing applies it any more. |
