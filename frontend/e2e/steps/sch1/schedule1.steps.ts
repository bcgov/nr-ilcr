...
Given('the read-only Schedule 1 anchor is an editable Draft', async ({ request, world }) => {
...
Given('the Schedule 1 anchor {string} is an editable Draft', async ({ request, world }, name) => {
...
Given(
  ...
  async ({ request, world, otherCostsCleanup }, name) => {
    ...
    expect(
      res.ok(),
      `precondition GET ${anchor.key.millId}/${anchor.key.year} -> ${res.status()}`,
    ).toBeTruthy()
    const doc = (await res.json()) as {
      trackStatus: string
      editable: boolean
...
Given(
  'the Schedule 1 render-state anchor {string} is selected',
 ...
 * a name promising all of them, so a regression that seeded the cost column, the silviculture rows, or
 * fired the BR-03 crown pre-fill would have passed (#296 code review).
 */
Then('every Schedule 1 amount is blank', async ({ schedule1Page }) => {
  const amounts = schedule1Page.allAmountInputs
  const count = await amounts.count()
  expect(count, 'the editable amount inputs must render at all').toBeGreaterThan(0)
  for (let i = 0; i < count; i += 1) {
    await expect(amounts.nth(i)).toHaveValue('')
  }
  await expect(schedule1Page.commentsInput).toHaveValue('')
})

/**
 * Defect #296 AC3 (legacy isScheduleOpen): nothing has been saved, so there is nothing to delete.
 *
 * DISABLED, not absent — this asserted `toHaveCount(0)` until the #296 code review caught it. Legacy
 * removed the button; defect #292 deliberately changed the MECHANISM to a disabled button plus a
 * visually-hidden reason, because an absent control and a dead one report differently to assistive
 * tech (see the comment block in `components/core/ScheduleActions/index.tsx`). The matching unit test
 * asserts `toBeDisabled()`, and this step must agree with it or it is red the first time it runs.
 */
...
When(
  'I enter {string} in the Schedule 1 {string} field',
  async ({ schedule1Page }, value, label) => {
...
Given(
  ...
  async ({ request, world, schedule1DeleteRestore }) => {
    ...
    expect(
      res.ok(),
      `precondition: GET Schedule 1 for ${millId}/${year} -> HTTP ${res.status()}`,
    ).toBeTruthy()
    const doc = (await res.json()) as {
      trackStatus: string
      editable: boolean
...
Then(
  ...
  async ({ schedule1Page }) => {
    ...
    for (const code of [12, 13, 14, 15, 16, 17, 18, 143, 144, 1, 2, 139, 140]) {
      await expect
        .poll(() => schedule1Page.amountValue(`#vol-${code}`), {
...
Then('the Schedule 1 {string} field should be empty', async ({ schedule1Page }, label) => {
