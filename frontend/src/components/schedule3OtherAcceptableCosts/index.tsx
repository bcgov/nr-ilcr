...
import type { OtherAcceptableErrors } from './validation'
...
const OtherAcceptableCostsPage: FC = () => {
  const { millId, year } = useMillYear()
  const navigate = useNavigate()
  const contextMissing = millId === null || year === null

  const [data, setData] = useState<OtherAcceptableDocument | null>(null)
  ...
  const [addErrors, setAddErrors] = useState<OtherAcceptableErrors>({})

  const [editingId, setEditingId] = useState<number | null>(null)
  const [editDescription, setEditDescription] = useState('')
  const [editTotal, setEditTotal] = useState('')
  const [editPop, setEditPop] = useState('')
  const [editErrors, setEditErrors] = useState<OtherAcceptableErrors>({})

  const [confirmDeleteId, setConfirmDeleteId] = useState<number | null>(null)

  const base = `/v1/schedule3/other-acceptable-costs`
  ...
  useEffect(() => {
    ...
    apiService
      .getAxiosInstance()
      .get<OtherAcceptableDocument>(`${base}${query}`)
      .then((response) => {
        if (active) {
      ...
      .catch((error: unknown) => {
        if (active) {
          setErrorDetail(extractDetail(error) || 'Unable to load Other Acceptable Costs.')
          setData(null)
        }
      })
      .finally(() => {
  ...
  const applyDocument = (doc: OtherAcceptableDocument) => {
    setData(doc)
  ...
  const handleAdd = () => {
    ...
    setMessage(null)
    setActionError(null)
    const errors = validateOtherAcceptable(addDescription, addTotal, addPop)
    ...
    apiService
      .getAxiosInstance()
      .post<OtherAcceptableDocument>(`${base}${query}`, {
  ...
  const startEdit = (row: OtherAcceptableRow) => {
    setEditingId(row.id)
    setEditDescription(row.description)
    setEditTotal(numStr(row.total))
    setEditPop(numStr(row.pop))
    setEditErrors({})
  }
  ...
  const handleSaveEdit = () => {
    ...
    setMessage(null)
    setActionError(null)
    const errors = validateOtherAcceptable(editDescription, editTotal, editPop)
    if (Object.keys(errors).length > 0) {
    ...
    apiService
      .getAxiosInstance()
      .put<OtherAcceptableDocument>(`${base}/${editingId}${query}`, {
  ...
  const handleDelete = () => {
    ...
    apiService
      .getAxiosInstance()
      .delete<OtherAcceptableDocument>(`${base}/${id}${query}`)
      .then((response) => {
        applyDocument(response.data)
      })
      .catch((error: unknown) => {
  ...
  if (isLoading) {
    return (
      <div className="app-page">
        ...
        <Grid fullWidth className="app-page__body">
          <Column sm={4} md={8} lg={16}>
            <LoadingScreen label="Loading Other Acceptable Costs" />
          </Column>
        </Grid>
      </div>
    )
        ...
        <Grid fullWidth className="app-page__body">
          <Column sm={4} md={8} lg={16}>
            <InlineNotification
              ...
              lowContrast
              hideCloseButton
              title="Unable to load Other Acceptable Costs"
  ...
  const rowCells = (row: OtherAcceptableRow) => {
        ...
        <>
          ...
          <TableCell className="schedule-3__num">
            <TextInput
              ...
              invalid={Boolean(editErrors.pop)}
              invalidText={editErrors.pop}
            />
          </TableCell>
          <TableCell className="schedule-3__num">{fmt(row.crown)}</TableCell>
          <TableCell>
    ...
    return (
      <>
        ...
        <TableCell className="schedule-3__num">{fmt(row.pop)}</TableCell>
        <TableCell className="schedule-3__num">{fmt(row.crown)}</TableCell>
        {editable && (
          <TableCell>
            <Button
    ...
    <div className="app-page">
      ...
      <Grid fullWidth className="app-page__body">
        ...
        <Column sm={4} md={8} lg={16} className="schedule-3__section">
          <TableContainer title="Other Acceptable Costs">
            <Table aria-label="Other Acceptable Costs">
              <TableHead>
                <TableRow>
          ...
          <dl className="schedule-3__summary">
            ...
            <div className="schedule-3__summary-item">
              ...
              <dd>{fmt(data.subtotal?.harvest)}</dd>
            </div>
            <div className="schedule-3__summary-item">
              <dt>Subtotal PO&P $</dt>
              <dd>{fmt(data.subtotal?.pop)}</dd>
            </div>
            <div className="schedule-3__summary-item">
              <dt>Subtotal Crown $</dt>
              <dd>{fmt(data.subtotal?.crown)}</dd>
            </div>
          </dl>
        </Column>
        ...
        {editable && (
          <Column sm={4} md={8} lg={16} className="schedule-3__section">
            <h3 className="schedule-3__heading">Add Other Acceptable Cost</h3>
            <div className="schedule-3-sub__add">
              ...
              <TextInput
                ...
                invalid={Boolean(addErrors.pop)}
                invalidText={addErrors.pop}
              />
