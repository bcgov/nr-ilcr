...
type UseCommittedValuesResult = {
  ...
  /** Commit one field (call from its `onBlur`). */
 ...
 * exactly when the server document, and with it the freshly seeded form, changes. Pass the same
 * `data` the page renders from; its IDENTITY is the signal, not its contents.
 */
export function useCommittedValues(form: FieldValues, data: unknown): UseCommittedValuesResult {
  ...
  const commit = useCallback((key: string) => {
    setCommitted((prev) => {
      const value = formRef.current[key] ?? ''
